...
| **M32 `fpround`** | **K23** (assigned 2026-09-18). The shared round-and-pack path — `soft_fround`'s `_sf_round_and_pack`, sticky/guard/round bits, the carry-out that bumps subnormal→normal **Widened 2026-09-18 (K15.md risk 5): M32 also owns the shared normalisation helpers `_sf_normalize_clz` and `_sf_handle_subnormal` (`softfloat_common.jl`), which `fadd`/`fmul`/`fdiv`/`fma` all call — assigned here so they are transcribed once as exported step blocks, not four times** **— and `_sf_normalize_to_bit52` too (K21.md, 2026-09-18): nine call sites in `fsqrt`/`fmul`/`fdiv`/`fma`/`flog`, and `fsqrt` calls it INSTEAD of the two above (`fsqrt.jl:27-29`, no subnormal result path)** | 260 | **PACK ↔ ROUND.** Upstream keeps them in one helper; at our LOC limit the rounding-decision logic splits from the field assembly. Seam is at the point the decision bit is known **K23.md DRAFTED 2026-09-18 (bead `9ve.35`, pre-implementation): four helpers, 154 rows (41 views); it records that PACK ↔ ROUND no longer partitions a four-helper module, and that M32 has NO host oracle so what L1 IS for it is a decision owed before `9ve.19` starts (K23.md §6.5 risk 1) — **DECIDED 2026-09-18 (§7.16): an IEEE reference model in `tests/support/`, written from the IEEE-754 rules and never from the Julia, plus the kernel-level host differential at M33/M35.** **LANDED 2026-09-19 (Wave 5, bead `9ve.19`): `fpround.{h,c}` + `fpround_step.c` + `fpround_emit.c` + `fpround_int.h` + `fpround_eval.c`. Seams taken: ROW TABLES ↔ STEP MACHINE, LAYOUT-AND-OPERANDS ↔ DISPATCH-AND-SURFACE (K23 D-K23-10, forced at 418 of 300), `_eval` ↔ THE CIRCUIT; the PACK ↔ ROUND seam above is RETIRED. Owns all four helpers; `_sf_normalize_to_bit52` has ELEVEN call sites in seven files; there is no `soft_fround`. 154 rows (41 views), 31,875 slots, 15,759 scratch at W=64, eight golden rows in `tests/goldens/fpround.counts`; M14/M16/M17 gained additive `cq_*_region(W)` / `cq_mux_steps(W)` exports so no consumer writes a region size down.** |
| **M33 `fadd`** | K15 — `soft_fadd` / `soft_fsub` | 280 | **ALIGN ↔ ADD-AND-NORMALISE.** The exponent-difference shift is the half that is `O(W)` in its own right and is the half `fma` reuses. **Do not spell `fsub` as `fadd(a, fneg(b))`** — upstream fixed a NaN-RHS sign bug in exactly that composition (`Bennett-m63k`) **LANDED 2026-09-19 (Wave 6, bead `9ve.21`): `fadd.{h,c}` + `fadd_step.c` + `fadd_operand.c` + `fadd_emit.c` + `fadd_eval.c` + `fadd_int.h`. Seams taken: ROW TABLES ↔ STEP MACHINE, LAYOUT-AND-OPERANDS ↔ DISPATCH-AND-SURFACE, `_eval` ↔ THE CIRCUIT, and a FOURTH forced at 326 of 300 — COSTS-AND-LAYOUT ↔ OPERAND RESOLUTION; the ALIGN ↔ ADD-AND-NORMALISE seam above is RETIRED as a file seam and kept as a marked boundary in the row table. 124 rows (19 views, 11 picks) for `fadd`, 133 for `fsub`; 47,815 / 48,836 slots; 23,585 / 24,033 scratch bits; four goldens in `tests/goldens/fadd.counts`. `fsub` is fadd's ROW PROGRAM under a nine-row prologue with `b` rebased onto `b_eff`, never a nested kernel; `cq_fsub_block` is exported for K19. M33's own body has NO signed compare and no `clamp` — both went to M32.** |
| **M34 `fmul`** | K16 — `soft_fmul` | 240 | **SIGNIFICAND PRODUCT ↔ EXPONENT/PACK.** The 53×53 product is M18's `mul` over a span; the seam is where the integer kernel ends. **Fixed 2026-09-18 (K16.md): the cut is at `fmul.jl:136`, after the 106-bit `(hi, lo)` assembly — the pair M39 reuses — not at `:81` after the four raw products, so all four `cq_mul_block` offsets stay in one file. Three K16 items settled by this document rather than in code: `ma & 0x03FFFFFF` is a VIEW (§7.3 as amended the same day — constant masks are wiring; this note first said BLOCK for three hours); `fmul.jl:94-102`'s abandoned bindings are TRANSCRIBED and marked dead, on §7.6's discarded-arm rule (the grain is mechanical, and 960 slots is the price of not deciding what is live); K11.md §7.5's `n_a·n_b` product identity does NOT hold inside M34, where a mask block's output is pre-materialised scratch (64 quantum lanes) — slots are unaffected, gates must be measured** **LANDED 2026-09-19 (Wave 6, bead `9ve.22`): `fmul.{h,c}` + `fmul_int.h` + `fmul_step.c` + `fmul_emit.c` + `fmul_eval.c`. Seams taken: ROW TABLES ↔ STEP MACHINE, LAYOUT-AND-OPERANDS ↔ DISPATCH-AND-SURFACE, `_eval` ↔ THE CIRCUIT; the `fmul.jl:136` seam is NOT taken as a FILE seam — it is the named row boundary `cq_fmul_seam_row()`, since the table is 128 lines so Rule 12 never binds and K20.md retired its M39-reuse rationale. 128 rows (35 views, 15 tuple projections, 6 M31 class rows), 163,300 slots, 44,060 bits, 294,692 gates forward and `_unc` alike, two golden rows in `tests/goldens/fmul.counts`; `:94-102` IS transcribed and marked dead (961 slots). Finding: the host `*` returns the FIRST operand's NaN payload at -O0 and the SECOND at -O1/-O2 on this box — §7.4's pin-by-table is load-bearing here, not only on arm64.** |
| **M35 `fdiv`** | K17 — `soft_fdiv` | 240 | **The 56-bit restoring-division loop ↔ pre-normalisation.** The loop is K12's shape and should **compose M19's exported step block, not transcribe it** (plan §0.4's obligation, and the K11/K12 composition-check trap applies verbatim) **K17.md DRAFTED 2026-09-18 (bead `9ve.35`, pre-implementation) and it measures that this row and §7.2 select DIFFERENT constructions: the loop's middle three lines are operator-for-operator K12's P1/P2/P3, its two ends are not, and it runs 56 times over 64-bit spans. K17.md §5.1 states both options. **DECIDED 2026-09-18 by the maintainer at the Wave 4 boundary (§7.16): TRANSCRIBE PER OPERATOR over M14/M16/M17 — §7.2's literal grain; the "compose M19's step block" instruction at the head of this row is RETIRED for `fdiv` exactly as K21 read it for M40. `9ve.25` implements K17.md's option B.** **LANDED 2026-09-19 (Wave 7, bead `9ve.25`): `fdiv.{h,c}` + `fdiv_step.c` + `fdiv_operand.c` + `fdiv_emit.c` + `fdiv_eval.c` + `fdiv_int.h`. Seams taken: ROW TABLES ↔ STEP MACHINE, COSTS-AND-LAYOUT ↔ OPERAND RESOLUTION, LAYOUT-AND-OPERANDS ↔ DISPATCH-AND-SURFACE, `_eval` ↔ THE CIRCUIT; PRE-NORMALISE ↔ LOOP is RETIRED as a file seam and kept as `cq_fdiv_loop_base_row()` (36). 478 rows (125 views, 15 projections) as a PRE table + a 7-row ITERATION TEMPLATE × 56 + a POST table; 139,245 slots, 64,659 scratch bits, 235,568 gates forward and `_unc` alike, two goldens in `tests/goldens/fdiv.counts`; the step machine walks 93 row costs per step. Findings: binary division CANNOT tie with a normal result (K17 §5.10's tie rows are subnormal quotients; its `exp_overflow_after_round` row is unreachable); `need_shift` is `ma < mb` on the SIGNIFICANDS; an inexact quotient does not anchor the sticky bit (a 29th anchor found by an independent integer model does); the host `/` FOLDS `0/0` and `Inf/Inf` to a positive qNaN where executing them gives x86's negative INDEF — §7.4's pin-by-table is load-bearing on this box.** |
...
| `shim/cq_template_fparith.c` / `shim/cq_template_unary.c` | the fp ABI WIRING — the `cq_template_*` symbols of `fadd`/`fsub`/`fmul`/`fdiv` at `f64` and the seventeen shipped conversion pairs, reached through the generator's `LANDED` rows | — | **LANDED 2026-09-19 (Wave 8, bead `9ve.36`)**, on two seams recorded in advance: `cq_template_fp.c`'s COMPARE ↔ ARITHMETIC (taken when it fired) and a third off `cq_template_impl.c`, ARITY-2 ↔ ARITY-1 (plan §3 Layer-5). Nine `cq_shim_fbin_*`, two `cq_shim_fcast*`, two `cq_shim_fun*`; three tag spaces (`0x10000000` fp binary, `0x20000000` fp unary, `0x30000000` fp conversion, recorded in `cq_template_boundary.h`). **A narrow conversion is a COMPOSITION at the shim on upstream's own shape** (`instructions.jl:7649-7658` emits the narrowing as a second IR instruction): the kernel at `T = 64` into a WORKSPACE rail then `cq_kernel_trunc`, or `cq_kernel_sext` then the kernel at `F = 64`, the workspace recorded as a FWD/UNC twin pair and freed through `cq_shim_free_proof` exactly as D7b's temporary is; `uitofp`'s narrow rows take `F` directly (§7.3). `uitofp i64 → f64` is DECLINED at the ABI edge under §7.9 / `9ve.34`, never routed to `sitofp`. The unary door exists for `fsqrt` (and M38's shape, §7.11) and is reached by no generated symbol until §6.1's vendoring; `fneg` and `frem` keep their aborts. Partition after this landing: 992 / 603 / 120 / 65 / 699 — read it from the generator |
...
**M32 `fpround`'s L1 oracle (K23.md §6.5 risk 1; bead `9ve.19`).** M32's four helpers are step
blocks with no host operator to differential against. **Decided: L1 for each block compares against
a hand-written IEEE-754 REFERENCE MODEL in `tests/support/` — sign, exponent, mantissa plus
guard/round/sticky, round-to-nearest-even, the subnormal→normal carry-out, overflow to Inf, the
flush-to-zero arm — written from the standard's rules and NEVER from the Julia source, exactly as
`ref_mux` is a plain selection rather than the kernel's identity; the kernel-level host differential
at M33/M35 is the end-to-end check on top.** Refused: a second C transcription of the Julia alone
(the Step 18 trap — an oracle sharing shape with the implementation is blind to what it gets
wrong) and deferring M32 to M33's differential (M32 would ship untested on its own and a wrong
helper would present as a kernel bug one wave later).

**`fdiv`'s loop (K17.md §5.1; bead `9ve.25`).** §5's M35 row said "compose M19's exported step
block"; §7.2 says one block per operator occurrence; K17.md measured that they select different
constructions (the loop's middle three lines are K12's P1/P2/P3, its two ends are not, and it runs
56 times over 64-bit spans). **Decided: TRANSCRIBE PER OPERATOR over M14/M16/M17's blocks (K17.md
option B)** — §7.2's grain, at W = 64 where every block is golden-covered, no gate list transcribed
so plan §0.4 / D9(e) hold, and about 2.5× cheaper than driving `cq_divrem_block` at W = 112, a width
the ABI never ships. The M35 row is amended in place; K21 had already read the M40 row this way.
