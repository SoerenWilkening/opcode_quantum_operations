...
| 4 | **Grover** — ordinary C compiles through `cqc`, links, emits a gate stream whose oracle arithmetic is verified exactly in classical mode. **AMENDED BY PRD §15 D22 (2026-08-28), and the amendment is that the two halves of this sentence RUN IN DIFFERENT MODES.** *Compiles / links / emits a gate stream* is the QUANTUM arm, through CQ_lang, opt-in behind `-DCQOPS_CQLANG_DIR=`, and it needs the **non-empty** clause spelled out — a program that folded away also links, runs and does not abort, i.e. also passes L6's D18 gate. *Verified exactly in classical mode* is the CLASSICAL arm, and what it verifies is the **value** plus **L5 at program scale** (zero gates, zero qubits) — **not** "the oracle's arithmetic circuits", which are never BUILT in that mode and are carried by condition 2 | 25 |
