...
### 0.5 A classical token is a SLOT STATE in M07, not a register and not a shim-side set

*(Added 2026-09-02 with PRD §15 D23, `bd 1za`. This is the one open design question tape
raised, and it is resolved here rather than in code because QRAM's array handle — `bd 9zq`,
Step 27 — reuses whatever is decided.)*

The frozen ABI has two handle kinds that are not rails: `cqrt_tape_alloc() -> t<N>` and
`cqrt_qram_alloc_<W>(count) -> a<N>`. Both are *classical resource tokens, like a file
descriptor* (`cq_runtime.h`), own zero qubits, are never tainted, are never freed, and are
**drawn from the same monotonic counter as every rail handle** — the goldens open
`cqrt_tape_alloc() -> t0` and continue `cqrt_alloc_i32(4) -> h1`. That last clause decides the
representation, because `tape`, `src` and `ctrl` are all `int32_t` and a token that did not
consume a number from M07's counter would collide with a rail's: *a rail handed as a tape*
and *a token handed to `cqrt_x`* would both be undetectable.

**Three candidates, measured against `src/reg.c` as it stands (238 LOC, 260 trigger, the
`reg_free.c` seam reserved):**

| candidate | why not |
|---|---|
| a width-0 register | `cq_reg_mint` refuses `width == 0` outright (`register width out of range`). Lifting that makes the token a RAIL: `cqrt_free` on it would run the disposition over zero bits and free CLEAN, `cqrt_measure` would return 0, and a template would accept it as an operand of width 0. Every one of those must be a hard error, so the token cannot be a degenerate rail |
| a shim-side token set | It cannot draw from the D5 counter without a slot, so it needs M07 anyway; and every rail refusal — 32 rail symbols, 30 gate symbols, fifteen `cq_shim_*` entry points, the §9 flag — would have to be re-implemented per entry point instead of inherited from M07's two funnels. That is *twelve places, not one* (risk R2) |
| **a fourth slot state, `CQ_SLOT_TOKEN`** — TAKEN | `width == 0`, `bits == NULL`, minted by `cq_reg_alloc_token(cq_reg_table *)`, queried by `cq_reg_is_token`. `cq_reg_slot` admits the state (so it is not the `0xAA` poison row); `cq_reg_readable` and `cq_reg_slot_mut` REFUSE it with one message naming the handle — and those two are the funnels every read, write, free and measure passes through, so every rail entry point in the shim refuses a token for free, in both configurations. `cq_reg_width` (0), `cq_reg_state` and `cq_reg_is_live` (false) stay permissive, which is what lets the I2 sweep and the D21 snapshot SKIP a token exactly as they skip a tombstone. `cq_reg_check_operands` refuses it BY NAME. **Cost: ~15 LOC in `reg.c`, landing under the 260 trigger; the `reg_free.c` seam stays reserved and untaken** |

**What the state carries is IDENTITY and nothing else.** A tape has no payload in v1.1 — the
ABI has no `tape_read`, and the backend's *tape inspection* is the kept rail itself, live at the
end of the program. QRAM will need one (`count`, element width, the entries): that goes in a
**shim-side side table keyed by the token handle**, never a field on `cq_reg`, and it takes the
**same** state — one token state, two owners, no fifth state.

**Where the token is NOT:** the recorder (no history, no mint marker; `cq_rec_hist(token)` is
NULL — a token is never written and never read), the D21 header (zero qubits, no `#REGISTER`
line — I4 made visible) and the D21 brackets (`cqrt_tape_alloc` joins `cqrt_alloc_i<W>`'s named
static exemption; a write's bracket names `src`/`ctrl` and `out`, never the token, because
`in=` would spell it `h<N>`).

---

### 0.6 QRAM: the array is the token plus `count` registers, the read is the flat unary-iteration tree, the store is the shadow store under that tree with ONE tape slot, and the certificate sees the token and the slot but never a cell

*(Added 2026-09-02 with PRD §15 D24, `bd 9zq`, Step 27 — the three design questions the
step raised, resolved here and in D24 before any file existed. D24 carries the argument;
this section carries what the module map and the tests rest on.)*

**(a) What an array is.** `cqrt_qram_alloc_<W>(count) -> a<N>` is §0.5's `CQ_SLOT_TOKEN` —
the SAME state a tape takes, out of the SAME counter, no fifth state — followed by `count`
ordinary M07 registers of width `W`, minted consecutively by `cq_reg_alloc_zero`
(handles `a+1 … a+count`), all-constant at birth (I4, zero qubits) and NEVER freed (the ABI
has no qram free). The token owns them through a shim-side payload table keyed by the
token handle (`shim/cq_shim_qram.[ch]`: width, count, first cell, the LIFO tape stack).
Three candidates, and the two rejected ones fail on things this plan already pins:

| candidate | why not |
|---|---|
| a shim-owned `cq_bit` array per array | L2 and I2 are stated over REGISTERS. `cq_reg_audit` and `cq_pc_live_is_exactly` would read a cell's materialised qubit as *live and owned by no named register*, and both would have to learn about the payload — a second ownership model beside M07's |
| one `count·W`-wide register | `CQ_REG_WIDTH_MAX` is 128 and the only integer corpus shape is `8 × 32 = 256` |
| **`count` registers behind the token** — TAKEN | L2's union form holds verbatim (a test's live set names every cell), the I2 sweep is untouched, `cq_reg_alloc_zero` takes the table so an alloc cannot emit (D21's static alloc exemption widens by one named family), and the D5 counter advances by `1 + count` — the numbering divergence D18 already retired |

**(b) Which construction, and its one delta each.** The READ (K13, `src/kernels/qrom.[ch]`) is
`emit_qrom!`'s unary-iteration tree (`qrom.jl:41-133`) with the leaf fan-out as a Toffoli per
(cell, lane) instead of Bennett's data-dependent CNOT — the fold table makes a constant cell
lane cost what constant data costs, so on a memset-0 array it IS the QROM gate for gate. It
is a Rule 7 kernel through `cq_sandwich`: compute = tree, copyout = fan-out, reverse = tree.
**The tree is FLAT (all `2·Lp − 1` flags live, BFS) where Bennett's is DFS (peak `2n + 1`)**,
because the driver's shape cannot interleave compute, fan-out and uncompute per node and
Rule 8 forbids hand-writing the per-node uncompute; gate count identical, qubit peak not,
DFS filed for v2 on D9's precedent. The STORE (K14, `src/kernels/qstore.[ch]`) is
`emit_shadow_store!`'s three CNOT sweeps (`shadow_memory.jl:38-55`) guarded per gate by the
leaf flag — `emit_shadow_store_guarded!` (`:98-121`) with `pred_wire = leaf_j` — under the
tree block K13 EXPORTS (M12/M19's pattern: call the sibling's step function, never the
kernel). **ONE tape slot per store rather than upstream's one per cell**: exactly one leaf
is set, so the per-cell slots' XOR is a single `W`-bit rail. K14 is NOT a Rule 7 kernel
(in place, destructive on cells and slot, inverse = reverse circuit — D17's shape) and still
runs under `cq_sandwich`: tree, sweeps, tree; push and pop differ only in the copyout's
index order. The controlled store is Rule 9 verbatim — `cq_shim_region`, every gate
promoted — and the cheaper root-flag guard is recorded in D24 and rejected.

**(c) The record.** The array TOKEN is minted in the record and is written by every store,
read by every load and store — because `out`'s certificate over `(load, load_unc)` reduces
the array's intervening writes, and a token with no history would let a store between them
release `out` DIRTY. The CELLS have no history: never an operand, never freeable by
CQ_lang; a cell reaching `cqrt_free` is UNPROVEN and answers to the shadow. The TAPE SLOT is
a recorded rail, born `|0⟩` at the push, written by push and pop as twins, and freed at the
pop through `cq_reg_free(T, cq_shim_free_proof)` — D15's disposition in one place: CLEAN iff
the pair reduces, STRANDED otherwise. The record widens from three operand slots to four
(`CQ_REC_SLOTS`), since a store names `arr, idx, val, T` and twins must carry identical
slots. The tape stack is per array, LIFO, and the pop VERIFIES the top's operands against
the call's — a mismatch or an empty stack is a hard error in both configurations.

**What this costs Rule 12, recorded BEFORE any file exists** (§3 has the rows and seams):
`src/kernels/qrom.[ch]` 160, `src/kernels/qstore.[ch]` 140, `shim/cq_shim_qram.[ch]` 140,
`shim/cq_runtime_qram.c` 220; `cq_shim_record.c` +6 rows toward its 240 trigger;
`cq_runtime_v2.c` 98 → 35 bodies; `cq_runtime_tape.c` +1 refusal (a qram token in the tape
slot — two owners of one state, and the tape file had no way to tell them apart).
...
| `refmodel.[ch]` | ~~150~~ **284 → split 2026-09-02** | Plain-C reference semantics per opcode at each `W`, with correct masking and two's-complement edge cases. **SPLIT on the ONE-WORD ↔ TWO-WORD seam (`bd zmo`)**: it had reached **284 of 300 with no seam recorded**, one line at a time, which is exactly what Rule 12 says must not happen. The seam is a subject cut the header had already drawn — below it every function takes or returns a `cq_ref_w`, above it none does, and the halves share exactly one symbol, `cq_ref_die`, now declared in the header. Measured after the split: **57 / 228**, no declaration moved |
| `refmodel_w.c` | **228 landed** | **Split from `refmodel.c` 2026-09-02** on the row above's seam: the two-word half (`cq_ref_w` masks, shifts, `icmp`, casts, add/sub, mul, div/rem) for the widths one `uint64_t` cannot reach — i80 and i128 |
...
| `kerneldrv.[ch]` | ~~249~~ ~~333~~ **279 → split 2026-09-02** (now 219 + 56) | **Added at Step 10.** The shared Phase-B driver §4 below assumes without naming — see the note. **SPLIT THREE TIMES, all on recorded seams — the third recorded and taken in one act (`bd f8c`)**: at Step 11 the sweep shapes went to `kernelsweep.c`; at Step 20 the controlled axis took the body to **345 of 300** and the axis went to `kernelctrl.c` (that seam is `the four LEVELS` against `the AXIS`, and it holds because `cq_kd_case` gained exactly three things from Step 20 — the control rail among the named registers, a control-aware L1 oracle, a per-mode L5 — while everything else the axis needed was new code with no assertion in it); and on 2026-09-02 the body stood at **279 with no third seam recorded**, one assertion from the wall — `bd zmo`'s situation arriving in the file next door. **The third seam is `the GATE` against `the INSTRUMENT`.** `cq_kd_case` ASSERTS — L1/L2/L3/L5, every line of it — while `cq_kd_measure` and `cq_kd_peak` assert NOTHING: their readings are pinned elsewhere, by the L4 goldens, by each suite's zero-ancilla claim and by `kernelctrl.c`'s promotion identity. It is a subject cut the file's own header had already drawn ("the four LEVELS live here" — L4 was never one of them, only its thermometer), and it was preferred over the bead's own candidate, the fixture plumbing ↔ the levels, which separates two things that are read together and exports the same five symbols for a worse reason. Both halves share the fixture and the one push site — `cq_kd_fixture`, `cq_kd_fx_open`/`_close`, `cq_kd_shape_of`, `cq_kd_call_kernel` — declared in `kernelfix.h` and defined in the gate half. Measured after the split: **219 / 58 / 16**, no public declaration moved, `kerneldrv.h` unchanged in every prototype |
| `kernelmeasure.c` | **58 landed** | **Split from `kerneldrv.c` 2026-09-02** on the row above's third seam: L4's instrument — `cq_kd_measure` (forward and `_unc` tuples, SEPARATELY, Rule 14) and `cq_kd_peak` — at the all-quantum fixture `measure_setup` builds once for both. Its two public entry points stay declared in `kerneldrv.h`, on `kernelctrl.c`'s precedent |
| `kernelfix.h` | **16 landed** | The internal joint the third split needed: the fixture and the one push site, declared once, defined in `kerneldrv.c`, included by nothing outside `tests/support/` — exactly `kernelctrl.h`'s shape |
...
| `goldens.[ch]` | **221 landed** | **Added at Step 10.** L4's counts as an on-disk artifact: load, check, regenerate, and the risk-R3 commit cross-check. Split seam: **parse/write ↔ check**, i.e. the file format leaves and `cq_gold_check` stays — trigger at 260 in `goldens.c`. **TEST SIDE, SEAM TAKEN 2026-08-29 at `bd 7ew`: `the REFUSAL ↔ the REMEDY` → `tests/test_goldens_update.inc`.** `tests/test_goldens.c` landed at Step 25 (`bd 2r5`) at **217 of 300** and 7ew's four cases are ~+75, which crosses the house 240 threshold in one step, so the cut was scheduled rather than improvised (Rule 12). It is a SUBJECT cut: every refusal the loader can make ends in the same sentence — *regenerate with `CQOPS_UPDATE_GOLDENS=1`* — so the parent keeps the refusals (a stale or missing `# measured-at:`, a re-pinned `# bennett:`, a missing file, a row nothing looked at) and the `.inc` keeps that instruction, executed. The discriminator is whether the case drives the UPDATE branch. It is the right half to name because **the remedy half is where the bug was**: at Step 10 the R3 mismatch fired before the writer, so the only way to obey the message the check printed was to hand-edit the pinned SHA — the exact laundering the check exists to prevent. The three pre-existing writer cases moved with it, so the seam is a subject and not a size cut |
...
Counts non-blank, non-comment lines. Runs as `make lint` — **which is two guards since `bd
0a7`, `check_loc.sh` then `check_cites.sh`** — and in CI, which was a statement of intent
until 2026-09-10 and is now `.github/workflows/ci.yml` (`bd kju`, `ubuntu-latest`, clang
only — decided 2026-09-10; a second compiler is a one-line matrix entry). **CI asserts no
file COUNT:** this script walks the filesystem rather than the git index and `tools/qtg/`
is gitignored, so the figure is box-local (measured 2026-09-10: 203 then 204 within one
session on the dev box, against 194 tracked). **SETTLED 2026-09-11 (`bd a9e`): the walk STAYS
on the filesystem and the COUNT is informational** — the filesystem set is a superset of the
tracked set, so nothing on the box escapes the limit, while an index-driven walk would stop
checking a brand-new module until it was `git add`ed, which is Rule 12 inverted. What changed
is the PRINTED line, which now names the set it counted. Re-measured that day: **204** walked
here against CI's **195** (clang job, run `34583512176`, `7cca77a`, green), the delta exactly
`tools/qtg/`'s nine countable files; the 194 above became 195 when `tools/l6/l6_report.py`
landed, and the delta itself has not moved. `check_cites.sh` has the identical shape (298 vs
288, its extra file being a `.sh`) and is filed as `bd 6kg`.
...
| M09 | `sandwich.[ch]` — the §0.1 driver. **Owns the scratch qubits end to end**: pre-materialises at step 0 (I6(b)) and releases in its own epilogue. I6 extent armed for the **compute halves only**. **Measured 2026-09-10 (`bd y0f`): 107 body + 10 header = 117 against 110, trigger 240, no split due** — 7 over budget and 123 lines short of the trigger, which is the ordinary landed state this table records for several modules. `bd y0f` filed it at 115 (105 + 10) on 2026-08-15 and that figure is CONFIRMED at the birth commit `1d9c8a0`, not superseded; the +2 arrived with M06 at Step 20 (`3f35def`), the only commit to touch either file since, and `sandwich.h` has not moved | 110 | the checksum helper moves to `scratch.c` — **never** the release, which would reopen "a kernel can call it". **Trigger 240** (`bd y0f`, on `reg.c`'s Step 7 precedent). **Measured 2026-09-10: the `#if CQ_SW_DEBUG` block — `sw_fingerprint` + `sw_verify` + the `SW_BASELINE`/`SW_VERIFY` macro pair — is 26 countable lines by `check_loc.sh`'s rule, not the ~20 the bead estimated, and is NOT free-standing: `cq_sw_die` is a file-static called from both sides of the cut** |
...
| M14 | `kernels/add.[ch]` | K6 add, K7 sub. **LANDED 2026-08-16 at 114 against 190**, so the seam is unused. **Step 17's export SHIPPED, at 124 body + 18 header** (plan §0.4, PRD §15 D9(e)): `k7_compute` and the sub half of `adder_env` are now `cq_sub_block` / `cq_sub_steps` / `cq_sub_step`, on M15's and M17's shape — additive, `cq_kernel_fn` untouched, and K6/K7 emit the same gates in the same order. **TEST SIDE, SECOND SEAM TAKEN 2026-09-10 (`bd iwn`): `the KERNEL ↔ its REFERENCE MODEL` → `tests/test_kernel_add_refmodel.inc`.** `tests/test_kernel_add.c` stood at **297 of 300** with only Step 12's `_upstream.inc` recorded, and the bead's two named candidates were **measured before cutting**: the `x+1`/`x+3` constant-increment cut is **SPENT** — all three constant-second-operand cases already live in `_upstream.inc`, so **0 countable lines move** — and the R8/R9 mask cut moves 85 to 213 but leaves `run_masked`, `gates_of` and `wide_ref` in the parent with **every caller inside an `.inc`**, and removes two of the three subjects the file's own header names as its reason to exist. What moved is the 48 lines of `ripple_ref` + `l1s_oracle_agrees_with_an_independent_bit_serial_model`, leaving **250**. **It is a subject cut and the discriminator is the sharpest in the file: that case runs no kernel, allocates no qubit and emits no gate** — it tests `tests/support/refmodel.c`'s `cq_ref_w_add`/`cq_ref_w_sub`, not `src/kernels/add.c` — so both halves stay a named subject instead of one becoming a residue. **It is also the house's own seam, already taken twice on material carrying the byte-identical section banner**: `tests/test_kernel_mul_refmodel.inc` (39) and `tests/test_kernel_divrem_refmodel.inc` (80), whose header says it in terms — "everything else in the parent file tests `divrem_u.c`, and this tests `refmodel.c`" — and **both of those suites had the R9 pair available instead and kept it in the parent**. Registration is unchanged (`.inc` files are `#include`d; the case stays last in `CQ_TEST_MAIN_ARGV`, as `mul`'s does). **RESERVE, RECORDED NOW BECAUSE 250 IS OVER THE HOUSE 240 TRIGGER: `R9's all-classical short-circuit ↔ everything that enters the sandwich` → `tests/test_kernel_add_classical.inc`**, 48 more (to 202), carrying `r9_all_classical_operands_never_enter_the_sandwich`, `classical_case` and `the_classical_fold_writes_real_gates_into_a_quantum_dst`, on `tests/test_kernel_cmp_derivation.inc`'s and `tests/test_kernel_mux_dispatch.inc`'s precedent of carrying exactly those cases. | 190 | `add.c` ↔ `sub.c` |
...
| **M29** | `kernels/qrom.[ch]` | **K13 QROM read at a quantum index (v1.2, PRD §15 D24, `bd 9zq`, Step 27)** — the FLAT unary-iteration tree as an exported step block (`cq_qtree_*`, the shape `cq_mux_step` set) plus the Rule 7 load `cq_kernel_qload(ctx, dst, idx, n_idx, cells, count, W)` — `dst ^= mem[idx]` — through `cq_sandwich`. **BUDGET 160, recorded before the file exists. Seam: `the TREE ↔ the LOAD`** → `kernels/qtree.[ch]`; discriminator: a line that names `dst` or a cell is the load's, a line that names only flags and `idx` is the tree's. Trigger 240. **LANDED 2026-09-02 at 175 (151 body + 24 header) against 160 — 9% over, on M19's precedent, and the seam is not owed at 151** | 160 | **tree ↔ load** |
| **M30** | `kernels/qstore.[ch]` | **K14 shadow store at a quantum index (same decision).** NOT a Rule 7 kernel (D17's shape): `cq_kernel_qstore_push` / `_pop`, in place on the cells and the ONE tape slot, driven by `cq_sandwich` over M29's tree with the three guarded sweeps as the copyout, push and pop differing only in the copyout's index order. **BUDGET 140. Seam: `the SWEEP STEP ↔ the ENTRY`** → `kernels/qstore_step.c`; discriminator: the step decode answers to `shadow_memory.jl`, the entry answers to D24's contract (the D7 check, the sandwich call, the direction). Trigger 240. **LANDED 2026-09-02 at 110 (98 body + 12 header) against 140** | 140 | **step ↔ entry** |
...
> define **173** `cqrt_*` symbols (65 in v1 scope — **76 since D23, 2026-09-02, with the 11 `tape`** — 34 fp aborts, 63 `qram`, ~~11 `tape`~~) plus
...
> | `shim/cq_runtime_rail.c` | the rail lifecycle: `alloc` / `measure` / `free` / `copy[_controlled]` / `cswap` / `addc` / `xorc` | ~~170~~ **LANDED 2026-08-23: 199** (193 at first draft; the adversarial review's fixes added six). **250 after `bd 76r`'s brackets (2026-08-28). The 240 trigger has fired; the split was REVIEWED AND DECLINED the same day (`bd 2z1`, closed) — 250 against Rule 12's 300 WALL, `check_loc.sh` green, and a trigger is a scheduling prompt rather than the limit. The `LIFETIME ↔ CONTENTS` seam stays recorded here and at the top of the file, with the two things it must not get wrong, for whoever next needs the room**. **268 since 2026-09-18 (`bd 9ve.28`): PRD-v2 §1's `f64` scope landed `cqrt_alloc_f64` / `cqrt_measure_f64` / `cqrt_copy_f64[_controlled]` here, by WIDENING what existed exactly as this table's `cq_runtime_v2.c` row predicted — `CQ_RAIL_COPY` and `CQ_RAIL_COPY_CTRL` take a TYPE TOKEN and a width separately, so `f64` is a row and not a copy of a function, and only `alloc`/`measure` are written out because their conversion is a memcpy where the integer widths need a modular narrowing. The seam is unmoved and untaken; 268 of the 300 WALL** | 220 |
> | `shim/cq_runtime_gate.c` | the gate surface: `x` / `cnot` / `toffoli` / `x_controlled` / `cnot_controlled` / `ry` / `rz` / `rz_controlled[_inv]` / `ry_controlled_inv` | ~~140~~ **LANDED 2026-08-23: 151** (156 at first draft; the review retired a guard whose comment was false). **210 since 2026-09-18 (`bd 9ve.28`): `CQ_GATE_ROT` and `CQ_GATE_ROT_CTRL` take a TYPE TOKEN and a width separately, and `f64` is two more invocations — TWO counted lines for six symbols, which is the "a family costs ~15 lines rather than ~90" argument above arriving one width at a time. Nothing in the file became fp-aware: a rotation is a statement about a WIRE, so §7's cells, D11's refusal and D12's poison are reached at `f64` by the same code as at `i64`** | 200 |
> | `shim/cq_runtime_v2.c` | everything v1 defers: 34 fp widths, 63 `qram`, ~~11 `tape`~~, `alloc_handle`. One abort shape, macro-expanded | ~~90~~ **LANDED 2026-08-27: 96**, and `bd r3y`'s worry that the 90 was written before either population was counted turned out to be unfounded — twelve macro bodies cover all 109. **98 bodies since 2026-09-02 (PRD §15 D23): the 11 `tape` bodies moved out, into the row below; 35 bodies since PRD §15 D24 later the same day, the 63 `qram` bodies moved out into `shim/cq_runtime_qram.c` — 44 lines**. **29 bodies / 49 lines since 2026-09-18 (`bd 9ve.28`): PRD-v2 §1's `f64` scope took the TEN f64 core symbols into the two rows above, leaving the fp core at 28 across f16/f32/f80. The seam below is the one that predicted this and its DEFERRED-CORE side is now three widths rather than four; `f80` additionally cannot be a memcpy at all (PRD-v2 §3.1: `long double` is an 80-bit value in a padded object), which is a second, independent reason it is last** | 150 |
| **`shim/cq_runtime_tape.c`** | **v1.1 (PRD §15 D23, `bd 1za`, 2026-09-02) — the 11 `cqrt_tape_*` symbols.** `cqrt_tape_alloc` mints a `CQ_SLOT_TOKEN` (§0.5) and opens no bracket; `cqrt_tape_write_<W>` is `cq_reg_alloc_zero` + the same `cq_reg_xor_into` `cqrt_copy_<W>` uses, returning the kept rail; `_controlled` is that inside the one `cq_shim_region` bracket, exactly as `cqrt_copy_<W>_controlled`. Two effect rows in `cq_shim_record.c`, with `src` a READ. **BUDGET 120, RECORDED BEFORE THE FILE EXISTS. Seam: `the TOKEN ↔ the WRITE`** — `cqrt_tape_alloc` and the token check on one side, the two write families on the other; discriminator: a line that touches a `cq_bit` is the write's. Moves to `shim/cq_runtime_tape_write.c`. Trigger 240, the house figure — and at this budget it is a formality: the file is one macro family of two bodies | ~~est. 70~~ **LANDED 2026-09-02: 89; 93 after D24's qram-token refusal** | 120 |
> | **`shim/cq_shim_qram.[ch]`** | **v1.2 (PRD §15 D24, `bd 9zq`, 2026-09-02) — the PAYLOAD TABLE.** What the token OWNS: element width, `count`, the first cell handle, and the per-array LIFO tape stack of `(slot, idx, val, pred)` entries; `cq_qram_alloc` (token + cells + the record's mint), `cq_qram_find` (NULL for a rail or a tape token — which is how `cq_runtime_tape.c` learns to refuse an array), `cq_qram_push` / `_pop` / `_top`, `cq_qram_reset` (called by `cq_shim_ctx_reset`, on `cq_rec_reset`'s precedent). **BUDGET 140, recorded before the file exists. Seam: `the ARRAY TABLE ↔ the TAPE STACK`** → `shim/cq_shim_qram_tape.c`; discriminator: a line that names a slot handle is the stack's. Trigger 240 | ~~est. 90~~ **LANDED 2026-09-02: 81 · 28** | 140 |
> | **`shim/cq_runtime_qram.c`** | **v1.2 — the 63 `cqrt_qram_*` symbols at all nine widths, an fp width being a bit-pattern cell (D24).** `alloc` opens no bracket (the named static exemption); `load` mints `out` at birth 0, records `QRAM_LOAD`, runs K13; `load_unc` is the same kernel with `dst = out`; `store` mints the slot, records, pushes, runs K14's push; `store_unc` verifies the top, runs K14's pop, frees the slot through `cq_shim_free_proof`, pops; the two `_controlled` families are those inside `cq_shim_region`. Refusals: a rail or a tape token as the array, an `out`/`idx`/`val`/`pred` that is not a rail of the width the symbol names, an empty or mismatched tape. **BUDGET 220. Seam: `the LOAD family ↔ the STORE families`** → `shim/cq_runtime_qram_store.c`; discriminator: a line that touches the tape stack is the store's. Trigger 240 | ~~est. 180~~ **LANDED 2026-09-02: 190** | 220 |
> | `shim/cq_template_impl.c` | the FIFTEEN remaining `cq_shim_*` entry points (the sixteenth, `cq_shim_unsupported`, is `cq_shim_ctx.c`'s): handle resolution, D7a refusal, D7b copy, minting `dst`, kernel selection, opening the region | ~~230~~ **LANDED 2026-08-27: 232, AND THE RECORDED SEAM WAS TAKEN** — the file came in at **270** against the 280 budget and the 240 trigger, which is Rule 12's "a scheduled split, never a surprise refactor" exactly. **A SECOND SEAM WAS TAKEN 2026-09-18 (`bd 9ve.28`) and is recorded in `shim/cq_template_boundary.h`: `the INTEGER opcode surface ↔ the fp opcode surface` → `shim/cq_template_fp.c`.** The file stood at 288 of the 300 WALL with `fcmp`'s four entry points (~35 lines) still to come. What crossed is deliberately small — `tpl_req`, `cq_tpl_req` and `cq_tpl_binary` — so D7a, D7b, the mint at the RESULT width, the §9 region, D15's twin record and D21's bracket are reached by an fp family through exactly the same code an integer one uses; a family owns only its kernel, its display name and a DISJOINT TAG SPACE. **288 after the move** (the `tpl_req` definition left, the include arrived) | 280 |
> | **`shim/cq_template_dispatch.[ch]`** | **LANDED 2026-08-27, on the seam recorded below before either file existed.** The three tables indexed by the frozen ABI's own enums — 13 opcodes, 10 predicates, 3 cast kinds — each with a `_Static_assert` tying its length to the last enumerator, each with a bounds check (C does not require an enum object to hold one of its enumerators, and an unchecked index calls whatever follows the table in `.rodata`), and **every `src/kernels/` include in the shim**. That last is the second argument for the cut: the boundary half names no kernel and so cannot quietly acquire a per-opcode special case. A cast carries its own function-pointer type — two widths `(F, T)` where a binary kernel has one `W` — because widening `cq_kernel_fn` to cover both is what Rule 7 forbids. **123 · 16 since 2026-09-18 (`bd 9ve.28`): a FOURTH table, `cq_tpl_fcmp_kernel`, plus its display names. It is a separate table and not a widening of `cq_tpl_cmp_kernel`, because `icmp` and `fcmp` overlap in four mnemonics (`ult`/`ugt`/`ule`/`uge`) and their yaml orders differ in thirteen of fourteen rows — a `(cq_fcmp_pred)pred` cast compiles clean and is wrong everywhere but `oeq`** | **57 · 11** | 280 |
> | **`shim/cq_template_boundary.h`** | **LANDED 2026-09-18 (`bd 9ve.28`) — the interface across `cq_template_impl.c`'s SECOND seam, `the INTEGER opcode surface ↔ the fp opcode surface`.** It carries `tpl_req`, the neutral builder `cq_tpl_req` and the ordered call sequence `cq_tpl_binary`, and NOTHING that can be got wrong silently: D7a, D7b, the mint at the RESULT width, the §9 region, D15's twin record and D21's bracket all stay inside `cq_tpl_binary`. It also records the FOUR TAG SPACES in use (binary, `icmp`, cast, `fcmp`) and says that a fifth family must claim a fifth — the tag is D15's twin identity, so two families sharing one lets an `_unc` of one pair with a forward of the other and discharge a free nothing uncomputed | **16** | 60 |
> | **`shim/cq_template_fp.c`** | **LANDED 2026-09-18 (`bd 9ve.28`) — v2's fp OPCODE surface: the four `cq_shim_fcmp_*` entry points M28's 56 live `cq_template_fcmp_*_f64*` wrappers call.** A family owns three things and this file has exactly those: its kernel table row, its display name and its tag space (`0xC0000000`). Plus one refusal — `bits != 64` is hard-errored HERE rather than left to `cq_kernel_fcmp_*`'s own width guard, which fires one layer down after the mint and after D21's bracket has opened. **Seam recorded before it is needed: `the COMPARE family ↔ the ARITHMETIC families` → `shim/cq_template_fparith.c`, trigger 100** — a compare's result width is one bit and an arithmetic one's is the operand width, and the arithmetic families carry the `_lh` shape and a controlled axis compares have on NO axis | **56** | 120 |
> | **`shim/cq_template_unary.c`** | **LANDED 2026-09-19 (`bd 9ve.36`) — the THIRD seam off `cq_template_impl.c`, `the ARITY-2 ordered call sequence ↔ the ARITY-1 ordered call sequence`, recorded in `shim/cq_template_boundary.h` at the same time as it was taken.** Forced by measurement: `cq_template_impl.c` stood at **288** of the 300 wall and the arity-1 sequence had to grow a WORKSPACE stage for the cross-domain conversions. Right by the discriminator the siblings use — the arity-2 half changes when a decision about ALIASING or CONTROL changes (D7b's copy, the §9 region, the literal lane, none of which an arity-1 op has), the arity-1 half when a family with a COMPOSITION lands. It carries `cq_tpl_unary` and the three INTEGER cast entry points, which came with it; `tpl_cast` left `cq_template_impl.c` (**288 → 247**). **What did NOT cross is a second copy of anything**: D7a's refusal, the three width doors, the mint, D15's record shape and D21's bracket are one implementation each, which is why `cq_tpl_width` / `cq_tpl_src` / `cq_tpl_out` are EXPORTED rather than re-spelled. **The outer stage's record names `a` and NOT the workspace** — `adjoint_matches` compares all four handle slots and a workspace handle is minted fresh on every call (D5), so naming it would make a forward and its `_unc` unpairable and strand every narrow conversion. **Reserve seam, recorded now: `the SEQUENCE ↔ the INTEGER cast entry points` → `shim/cq_template_cast.c`, trigger 240** | **120** | 300 |
> | **`shim/cq_template_ternary.c`** | **LANDED 2026-09-19 (`bd 9ve.24`) — a FOURTH seam off `cq_template_impl.c`, `the ARITY-2 ordered call sequence ↔ the ARITY-3 one`, recorded in `shim/cq_template_boundary.h` before it was taken.** ONE family reaches it: `fma`, the only `arity: ternary` row in the `intrinsic_table.yaml` PRD-v2 §6.1 vendors, in four shapes (`qqq` bare, `qql`, `qlq`, `qll`) with `_unc` twins and **no `_controlled` and no `_inv`**. **FOUR things arity 3 changes and nothing else does.** (i) TWO literal lanes — `tpl_req` carries one `(lo, hi)` pair and `qll` needs two, so one shared buffer would make `fma(a, 2.0, 3.0)` compute `fma(a, 3.0, 3.0)`. (ii) D7b has THREE source pairs and `cq_reg_sources_alias` answers a bare 0/1, which at arity 2 is exact and here cannot distinguish `a == b` (ONE temporary) from `a == b == c` (TWO) — so this file does its own pairwise scan and the D21 prediction is `cq_reg_count + n_temps`. (iii) NO §9 region, because that ABI row has no controlled variant. (iv) D15's record fills ALL FOUR of `CQ_REC_SLOTS` — which is why `CQ_ROP_TPL_FWD`/`TPL_UNC` had to start READING slot 3, a slot-shape widening that is sound for every arity-1 and arity-2 producer because both set `h[3]` to `CQ_REG_NONE` and every consumer guards `h < 0`. **EIGHTH tag space, `0x50000000`, with the boundary header's arithmetic re-checked rather than assumed.** **What did NOT cross is a second copy of anything**: the three width doors, `cq_reg_check_operands`, `cq_tpl_rec_copy` (newly EXPORTED for this reason) and `cq_shim_free_proof` are one implementation each. **Reserve seam, recorded now: `the SEQUENCE ↔ the eight entry points` → `shim/cq_template_fma.c`, trigger 240** | **115** | 300 |
> | **`shim/cq_template_fparith.c`** | **LANDED 2026-09-19 (`bd 9ve.36`) — the seam `shim/cq_template_fp.c` reserved, taken when it fired.** The nine `cq_shim_fbin_*`, the two `cq_shim_fcast*` and the two `cq_shim_fun*` entry points; three TAG SPACES (`0x10000000` fp binary, `0x20000000` fp unary, `0x30000000` fp conversion); the hoisted width refusals; the SEVENTEEN shipped conversion pairs, with `uitofp`'s source list ASKED of `cq_fconv_uitofp_widths` rather than transcribed; and **bead 9ve.34's `uitofp`-from-i64 refusal at the ABI edge** — `cq_kernel_uitofp` refuses it too, one layer down, after the mint and after D21's bracket has opened. The NARROW conversion rows are a COMPOSITION on upstream's own shape (`instructions.jl:7649-7658` emits the narrowing as a second IR instruction), expressed as `tpl_ureq`'s inner/outer stages and not as a second sequence | **145** | 300 |
...
> same reason. `tests/support/refmodel.c` WAS the standing counter-example: **284 of 300 with
> no seam** (`bd zmo`), and it got there one line at a time — **split 2026-09-02** on the
> one-word ↔ two-word seam its header had already drawn, into 57 + 228 (§2.2).
>
> | file | seam | moves to | trigger |
> |---|---|---|---|
...
| M27 | `shim/gen_shim.py` | 280 | Reads `opcode_table.yaml`; emits one `.gen.c` per opcode family, **plus BOTH abort buckets — the 884 fp AND the 603 integer `_inv` bodies (PRD §15 D14, `bd w1c`)**; the pre-D14 wording said "fp aborts" and named only half the bucket. **Split seam recorded 2026-08-22, before a line is written (Rule 12: a split is scheduled, never improvised): `the ABI grid ↔ the emitted bodies`.** `gen_shim.py` keeps everything the frozen ABI dictates and stays the path CI invokes — the yaml load and its sha, the `:35-59` naming-rule expander (**`:39-58` until 2026-08-22 — it starts four lines INSIDE the block, omitting `:36-38`'s three forward-form rules, i.e. the base symbol shape itself; a generator built from it has no `fwd_qq`/`_hl`/`_lh` spelling at all**), the row model, `signature(row)` (with the classical operand's `c_type` read out of the yaml's own `widths` map, never a hand-kept table), each symbol's v1 bucket, PRD §1's partition audit, R7's per-family bucketing, the banner and `--check`. `shim/gen_bodies.py` takes a row and returns the text between the braces, and is **the only file in M27 that names a libcqops or M26 symbol**. Import is one-way; the row record is the whole interface. The seam is where Step 22's gate stops and Step 23's begins, and it is what lets the round-trip oracle — 2479 names AND 2479 signatures diffed against CQ_lang's own generated `runtime/cq_templates.h` — run from the grid half alone with no body emitted. **Trigger it at 240**, the house threshold (M07, M06, M20, M22). **LANDED 2026-08-22 AT STEP 22, PRE-SPLIT, AND THE PROTOTYPE'S FIGURE WAS THE GOOD ESTIMATE: `gen_shim.py` **183** and `gen_bodies.py` **56** under `check_loc.sh`'s py rule — 239 together against the prototype's 190 and the "plausibly ~290" caveat.** The TEST side is where the room went instead, and its seam is recorded in its own header: `tests/test_gen_bodies.py` reached **259 of 300** after an adversarial review, and `the ASSERTIONS ↔ the PROVOCATIONS` moves to `tests/test_gen_bodies_provoked.py` — a THIRD registered ctest test, because Python has no `.inc` escape hatch. The split was taken **before** the trigger and that is the plan working as written rather than an exception: this row already calls the seam's case *ownership, not churn*, and the ownership boundary is exactly where Step 22's gate stops and Step 23's begins. Three artefacts landed with it that the row did not anticipate and that are recorded here so they are not re-derived: **`shim/cq_shim.h`** (40 lines) is the M26↔M28 *contract* — a generator cannot emit calls into a vacuum, so Step 22 ships the sixteen declarations and Step 23 ships their definitions; **`tests/abi/cq_templates_abi.txt`** is the round-trip oracle *vendored* rather than read live out of an unpinned CQ_lang, carrying the yaml-sha256 that proves it is the expansion of the yaml we actually pin (CQ_lang's HEAD has already moved past `third_party/cq_lang/COMMIT` while the yaml has not); and **`cmake/CqopsPython.cmake`** probes for a python3 that can `import yaml` in the compile-and-*run* style of `CqopsSanitizers.cmake`, warning and declining to register the two suites rather than registering them to pass — the reserve seam below is what the warning is warning about. **Reserve seam, grid side: `the reader ↔ the expander`** — PyYAML is present on this box's `python3` and absent from `/usr/bin/python3`, so if CI forces a hand-rolled reader it lands here (~+60–90) and moves to `shim/yamlmin.py`. **Amendment 2026-09-10 (`bd kju`): CI did NOT force it.** `.github/workflows/ci.yml` installs the distro `python3-yaml` and configures with `-DCQOPS_PYTHON=ON`, so a missing PyYAML is a hard configure error there rather than two silently unregistered suites — the seam stays in reserve, and the reason it was ever at risk (a runner with no PyYAML) is now a red configure instead of a green run. Two riders: `check_loc.sh` counts every line of a multi-line string literal as code and Python has no `.inc` escape hatch, so emitted C templates cost the bodies half a line each — a second, independent reason that half grows first; and a re-pin costs **no code on either side** (simulated: a new width, predicate and cast pair took 2479 → 2528 with zero edits), so the seam's case is ownership, not churn. **THE TEST SEAM WAS TAKEN 2026-09-18 (`bd 9ve.28`), exactly where this row records it**: PRD-v2's first fp family took `tests/test_gen_bodies.py` from 259 to **319** of the 300 WALL, so `the ASSERTIONS ↔ the PROVOCATIONS` moved to `tests/test_gen_bodies_provoked.py` — the third registered ctest entry this row predicted, listed in `tests/CMakeLists.txt`'s `foreach(pysuite …)` loop. The generator side grew by a **LANDED table** (`gen_shim.LANDED`, one `(family, width)` pair per shipped fp kernel) plus an entry-key change in `gen_bodies.py`, and `gen_shim.py` **206** / `gen_bodies.py` **72** are the measured figures. The next seam on the test side is recorded in its header: `the STATIC reading ↔ the EXECUTED witnesses`, trigger 260 |
| M28 | generated `*.gen.c` | exempt | ~~**992** integer wrappers + **603** integer `_inv` abort bodies + **884** fp abort bodies~~ (PRD §1 and §15 **D14**; the 1595 integer symbols split 992/603 because `_inv` is `f⁻¹` and `f⁻¹` does not exist for `and`/`or`/`udiv`/`trunc`/`icmp`. 1455 integer if i80 is ruled out of scope). **SINCE 2026-09-18 (`bd 9ve.28`) THE PARTITION IS 1048 / 631 / 800**, and it is no longer "integer / integer / fp": a family-width listed in `gen_shim.LANDED` takes the ORDINARY bucket rule, so `wrapper` holds 992 integer + **56 fp** and `inv` holds 603 integer + **28 fp** (an `fcmp` is non-injective at every width and in every version, so D14 refuses its `_inv` whether or not its forward ships). **Read the figures from `gen_shim.py`'s `EXPECTED` and `EXPECTED_BY_DOMAIN`, which the audit enforces, never from this cell** |

Hand-written total ≈ **3,400 LOC** across 27 modules.

### Layer 6 — the L6 and L7 harnesses *(Steps 24–25; not library modules, and `tools/` IS linted)*

`tools/check_loc.sh`'s roots are `src include tests shim tools`, so a `.py` under `tools/`
is subject to Rule 12 exactly as a kernel is. This paragraph read **"Both seams below are
recorded before either file needs one"** until 2026-09-10 (`bd wgi`). The recording half
stayed true — the `l6_run.py` seam was on this table at 142, and it was that recorded seam
that was TAKEN on 2026-09-10, late rather than in advance, at 281 against a trigger of 240
— and the count half is what went stale: the table is four files now, the third recording
no seam by decision (its row says why) and the fourth, `tools/l7/l7_run.py`, recorded **in
advance** on 2026-09-11 against the same trigger of 240 (`bd ss8`), which is the recording
half working as written.

| file | LOC | Split seam |
|---|---|---|
| `tools/l6/run_slice_cqops.sh` | **146 raw**, of which **47** are non-blank and non-comment (shell is not counted by `check_loc.sh` at all — its `find` matches only `.c`/`.h`/`.py`). This cell read **106 raw** until 2026-09-11 (`bd ss8`): that was measured when the row was written and the file has grown under it | Not applicable, and deliberately so: this is CQ_lang's own four-stage pipeline with ONE line changed — the link — and the whole value of it is that it stays diffable against `CQ_lang/tests/e2e/run_slice{,_multi}.sh`. Splitting it would defeat the review that keeps it faithful. **Shared with L7 since Step 25, and it is the ONLY thing L7 shares with L6**: `tools/l7/l7_run.py` reaches it by path and imports nothing from either L6 module (the row below says what it duplicates instead, and why that is the precedent rather than a defect) |
| `tools/l6/l6_run.py` | **142 of 300** (landed 2026-08-27). **RE-MEASURED 281 on 2026-09-10 (`bd wgi`), past the trigger; SPLIT TAKEN the same day → 177** | **`the FIXTURE LIST ↔ the VERDICT`** → `tools/l6/l6_report.py`, trigger **240**. The fixture half reads `ctest --show-only=json-v1` out of an UNPINNED CQ_lang and answers to whatever CQ_lang registers; the verdict half (`classify`, the `DEFERRED` regex, the residue observation) answers to OUR abort texts in `shim/cq_shim_ctx.c` and `src/reg.c`. The discriminator is **which repository a line breaks with**, and no line answers to both — the same test this table applies to `shim/cq_shim_record.c`. Rejected: `run ↔ report`, which sounds like the same cut and is not, because `--reuse` already makes the report runnable without the run and a cut there would only move `main`'s loop. **TAKEN 2026-09-10 (`bd wgi`), AND THE SEAM CUT WHERE THIS ROW SAID IT WOULD** — measured with `check_loc.sh`'s own rule, 281 → **177** here + **143** there. What moved is exactly what this row names — `DEFERRED`, `STRAND`, `classify`, `RESIDUE_FIELDS`, `residue`, `residue_totals`, `residue_summary` — plus the two docstring paragraphs that answer to our abort texts, and the three text helpers `read` / `first_line` / `first_error`. **`main` did NOT move**, which is the rejected cut staying rejected. **ONE WRINKLE THE DISCRIMINATOR DOES NOT DECIDE**: those three helpers answer to NEITHER repository, so they followed their callers — and because `residue()` needs `read()`, `l6_run.py` imports it back. That is the one crossing, and it is in the sanctioned direction. The import is ONE WAY on `gen_shim.py`/`gen_bodies.py`'s precedent with `sys.dont_write_bytecode = True` set BEFORE it, and that line is the WHOLE defence rather than decoration: measured 2026-09-10, the L6 and L7 ctest registrations do **not** set `PYTHONDONTWRITEBYTECODE` — only the Step-22 shim suites do — so nothing else stops a stale `l6_report.pyc` serving a previous `classify`. **Behaviour is byte-identical**: pre- and post-split code interleaved over one artefact set of 342 fixtures gave identical `--json` and identical stdout, and `--only`/`--gate`/`--min-ok` agree on both the rc-0 and the rc-1 path |
| `tools/l6/l6_report.py` | **143 of 300** (split out 2026-09-10, `bd wgi`) | **No seam recorded, and the reason is not just the margin.** At 143 against the house **240** it is nowhere near a cut; more to the point the repository discriminator is EXHAUSTED here — every line in the file breaks with *this* repository, which is what makes it one half rather than two. A future cut therefore needs a different discriminator, and the one the documents already draw is **PRD §15 D18's**: the verdict owns the exit code, the residue is read ALONGSIDE the gate and never as one. Record it properly if this file passes ~200 |
| `tools/l7/l7_run.py` | **217 of 300**, and the figure MOVED INSIDE ONE DAY, which is recorded rather than smoothed over: **214** measured 2026-09-10 (`bd wgi`) with `check_loc.sh`'s own rule, **CONFIRMED unchanged at 214** on 2026-09-11 (`bd ss8`), then **217** later that same day when `bd y4o` restored the three lines that verify `ar x` actually produced each named member. The largest countable file under `tools/`, the largest TRACKED one there, and 36th of the 204 files `check_loc.sh` walks on this box. `wc -l` says **308** and is the wrong rule: **61** of the 217 are the module docstring, which `check_loc.sh` counts as code deliberately — over-counting is the safe direction for a guard | **`the PIPELINE VERDICT ↔ the GATE-STREAM VERDICT`** → `tools/l7/l7_verdict.py`, trigger **240**, recorded 2026-09-11 **in advance**, with **23** lines of headroom. The file states SIX numbered clauses and they split exactly in two: **clauses 1–3 are PRD §15 D18's gate verbatim** — it lowers, it links against this archive alone, it runs and does not abort — and every line that decides them reads an ARTEFACT `run_slice_cqops.sh` wrote, by existence or by exit code (`.rc`, `.prov`, `.link`); **clauses 4–6 are L7's extra claims** — the gate MIX, classical mode emits nothing, no phase on a measured rail — and every line that decides them reads the TEXT of the printf sink. The discriminator is therefore the one this table already applies to `l6_run.py` and to `shim/cq_shim_record.c`, **which repository a line breaks with**: an artefact-or-exit-code line breaks with CQ_lang and with the shared shell runner, a text line breaks with M23's `q<N>` spelling in THIS repository — `GATE`, `RZ_TARGET` and `MZ_TARGET` are M23's format and nothing else — and no line answers to both. **Measured 2026-09-11 with `check_loc.sh`'s rule: ~123 stays, ~90 moves.** Stays — the pipeline docstring paragraphs (6 + 10 + 4 + 4), `cq_revision` (6), `toolchain` (16), `extract_intrinsics` (**24**, and 21 before `bd y4o`), `main`'s argparse, setup and banner (16), the stale-artefact sweep (4), the runner invocation (5), the provenance and DID NOT BUILD arms (12). Moves — the clause 4/5/6 docstring paragraphs (14 + 17 + 6), `GATE`/`REQUIRED`/`RZ_TARGET`/`MZ_TARGET` (4), `gate_mix` (5), `tail_value` (6), `report` (12), and the three clause bodies lifted out of `main` as functions over text (5 + 7 + 4). Taken at the trigger that is roughly **135 + 105**. **Three obligations inherited from `bd wgi`'s landed split.** The import is ONE WAY with `sys.dont_write_bytecode = True` set BEFORE it — measured 2026-09-11, the L7 ctest registration sets only `CQOPS_L6_LDLIBS` / `CQOPS_L6_LDFLAGS` / `CQOPS_L6_LINK_CC` and does **not** set `PYTHONDONTWRITEBYTECODE`, so that one line is the whole defence against a stale `l7_verdict.pyc` serving a previous verdict. One-way is WEAKER here than in L6 and must be written down rather than assumed: `l6_run.py` ends in a module-level `sys.exit(main())`, so importing it runs the corpus and the dependency cannot be reversed by accident, while `l7_run.py` carries an `if __name__ == "__main__":` guard and nothing structural stops the reverse import. And `read` answers to NEITHER repository, so it follows its caller into the verdict half and `l7_run.py` imports it back — the one sanctioned crossing, exactly `l6_run.py ← l6_report.read`. **THE HOUSE VALIDATION IS UNAVAILABLE HERE, AND THE CUT SUPPLIES ITS OWN REPLACEMENT.** `bd wgi` was validated by re-classifying the SAME artefacts with both versions interleaved, which `--reuse` made possible; `l7_run.py` has no `--reuse` and deletes every artefact unconditionally on entry (its own *"a stale .rc reports a stale run"*), so two runs against an UNPINNED CQ_lang is all that is left and is exactly what that memory forbids. After the cut the verdict half takes TEXT rather than paths: capture one run's `grover.out` and the classical arm's stdout once, then drive pre- and post-split verdict code over the same two strings and require byte-identical stdout and `--json`. **Three rejected cuts, named so they are not re-proposed.** (i) the THREE §12 CLAIMS (PRD §15 **D22**) is SPENT — the partition is already ACROSS files, §12(2)'s pinned half in `tests/test_grover.c` and §12(3)'s T-count in `tests/test_grover_qec.c`, and the only trace of it inside this file is clause 5, measured **10** countable lines; 10 of 217 is a rename, not a seam. (ii) the DOCSTRING ↔ the CODE moves 61 lines and has no subject; under the seam above those paragraphs move only because their clauses do. (iii) **Hoisting the three shared helpers into a module both directories import** removes **36** countable lines and adds an import and a `sys.path` line, so about **34** net (217 → ~183) — and it buys no second subject, which is what a seam is for; it also runs against this table's own precedent that a small helper is duplicated deliberately rather than put one indirection away from the caller that knows the operand order (M26's two `rec` helpers; `l6_report`'s `read` / `first_line` / `first_error` following their callers). **`bd ss8` recorded that hoist as buying 27 lines and leaving the file *still past ~200*; both halves were wrong** — the arithmetic predates `bd y4o`'s +3 and 183 is not past 200 — so the objection that survives is the missing subject, not the margin. **THE DUPLICATION IS DELIBERATE; ITS ONE DRIFT WAS A DEFECT AND NOT A SEAM, AND IT IS FIXED (`bd y4o`, 2026-09-11).** Measured that day: `cq_revision` (6) and `read` (6) are BYTE-IDENTICAL to `l6_run.py`'s and `l6_report.py`'s, and `extract_intrinsics` **HAD** diverged — 21 lines here against 29 there, the L7 copy lacking the three that verify `ar x` actually produced each NAMED member, which is `bd 216` item 20's failure mode with the detector removed, in the one harness that has no `--reuse` to notice it. The three lines were restored in place the same day, the assertion was observed to fire against a stub `ar`, and the two code halves now differ in their docstrings and in nothing else. That restoration is the whole of 214 → 217 |
...
| 27 | **v1.2 — QRAM (`bd 9zq`, PRD §15 D24, §0.6, 2026-09-02).** ~~*(stretch)* port `qrom.jl` and `softmem.jl`~~ — `softmem.jl` is Bennett's Julia-level MUX model with no gate construction in it; the gate-level store is `shadow_memory.jl`, measured. The 63 `cqrt_qram_*` symbols move from `cq_runtime_v2.c`'s aborts into `shim/cq_runtime_qram.c` over `shim/cq_shim_qram.[ch]` (the payload table); M29 `kernels/qrom.[ch]` (K13, the flat unary-iteration tree + the Rule 7 load) and M30 `kernels/qstore.[ch]` (K14, the shadow store at a quantum index, NOT Rule 7); six record rows and a fourth operand slot in `cq_shim_record.[ch]`; `cqrt_qram_alloc_<W>` joins the D21 alloc exemption; `cq_runtime_tape.c` refuses a qram token. **Red** — `tests/test_kernel_qrom.c` (+ `_death`): K13 through the SHARED driver with the cells as ONE `count·W`-bit source and a `call` adapter that carves it into `count` pointers (so `count·W ≤ 128` on that path), refn = plain C indexing at the truncated index; L4 goldens `tests/goldens/qrom.counts` keyed `qload_L<count>` per `W` over `count ∈ {1,2,3,4,8,16}`, the corpus shape `(8, 32)` and `(16, 64)` measured OUTSIDE the driver on real cells; the palindrome; the COMPOSITION check that asks the tree block what it costs (`cq_qtree_*` under a counting sink) and pins `load = 2·tree + count·W` — reads no golden (the K11 lesson); the death cases (`n_idx < n`, an aliased cell, `count ≤ 0`). `tests/test_kernel_qstore.c` (+ `_death`): the levels restated by hand as K8's are — L1 push: `cell[idx'] == val`, `T == old`, every other cell unchanged, and at a padded index NOTHING moves; L3 = pop restores every cell and `T == 0`; L2 as the set including every cell and `T`; L4 `qstore.counts`; the composition check `push = 2·tree + 3·count·W`; the §9 promotion identity through `cq_ctrl_push`. `tests/test_runtime_qram.c` (+ `_death` + a sweep `.inc`): the token (`a`, then `a+1 … a+count` live at width `W`, zero qubits, zero gates); L1 over random `(idx, contents, val)` at `cq_kd_samples()` per width through the ABI with `cq_bk_reg` operands; L2 the set including every cell and the slot after every call; L3 `load` → `load_unc` → `out` is 0 and `cqrt_free(out)` releases CLEAN with 0 strand reports; `store` → `store_unc` restores every cell AND the slot is released CLEAN by the CERTIFICATE with a rotation-poisoned index (the shadow cannot see it — that is the clause D24 (c) exists for); the LIFO order over two nested stores; `a_store_between_a_load_and_its_unc_strands_the_out_rail` (the unsound face, asserted as a strand); the controlled rows under ZERO / ONE / Q0 / Q1; the six corpus shapes replayed end to end through the ABI with real `cqrt_ry` indices, running to completion with the residue counted; the deaths (a rail as the array, a TAPE token as the array, an array token as a tape, `count ≤ 0`, a wrong-width `out`/`idx`/`val`, a pop on an empty tape, a mismatched pop). **Gate** — both configurations green, `make lint`, a mutation battery per the recorded harness rules with a must-die and a must-crash first, and L6 in both trees: the `qram is v2` bucket falls to ZERO, the six integer fixtures move to OK (or name where they moved), the fp-width qram fixtures move to `fp is v2`, the CQ_lang SHA and dirty state named. **LANDED 2026-09-02.** `src/kernels/qrom.[ch]` **151 · 24** of 160, `src/kernels/qstore.[ch]` **98 · 12** of 140, `shim/cq_shim_qram.[ch]` **81 · 28** of 140, `shim/cq_runtime_qram.c` **190** of 220 (check_loc's non-blank, non-comment counts); `cq_shim_record.c` **201** (+6 rows and the fourth slot), `cq_shim_reduce.c` 112 (its three slot loops now run to `CQ_REC_SLOTS`), `cq_runtime_tape.c` 93 (+1 refusal); `cq_runtime_v2.c` down to **35 bodies** in 44 lines; `docs/cqrt_census.txt`'s v1 list 139, `nm` still 171. **Tests: 29 ctest entries** — `test_kernel_qrom` + 6 deaths, `test_kernel_qstore` + 5 deaths, `test_runtime_qram` + 15 deaths — plus the D24 case in `test_shim_trace`; each suite's large tables live in an `.inc` (`_cells`, `_measure`, `_sweep` + `_corpus`), and the goldens `tests/goldens/qrom.counts` / `qstore.counts` are keyed `qload_L<count>` / `qstore_L<count>` at `W ∈ {1, 8, 16, 32, 64, 80}`, regenerated in Release. **RAN, this tree: Release 354/354, Debug 355/355 (ASan + UBSan + LeakSanitizer; L6 excluded from both counts and run separately), `make lint` green (193 files, largest 297).** **BATTERY: 34 mutants + 2 sanity (must-die, must-crash) over the six files the runner backs up, all 36 LANDED, 0 NOOP, 0 NOCOMPILE, in both configurations.** Release: 35 KILLED, 1 SURVIVED — **E3**, the `QRAM_STORE_UNC` row's `twin_of`, is INERT by the engine's own scan (the reduction reads the EARLIER row of a pair, so the pop row's twin is never consulted while the LIFO check holds) and was then pinned as declared intent by `the_six_record_rows_are_what_d24_declares`. Debug, run AFTER that pin: **36 KILLED, 0 SURVIVED** â E3 dies to the row pin (`test_runtime_qram`), and every other verdict and killer list is identical to Release's, so nothing in this battery is Debug-only. Beyond those two the kills are the same instruments in both configurations — the K-series by the kernel suites' L1/L4/composition checks, the S-series by `test_kernel_qstore` and the ABI sweep, the R/Q/E-series by `test_runtime_qram`'s certificate cases and the deaths, `R12` by `test_shim_trace` alone. **L6, BOTH TREES IDENTICAL, against CQ_lang `893b769` (2026-08-30, DIRTY working tree, 92 modified or untracked paths), 274 registered fixtures: OK 71 (was 65 — exactly the six integer qram fixtures moved, each with ZERO residue in every grain), DEFERRED 200 (`fp is v2` 194, which now holds the 21 fp-bearing former `qram is v2` fixtures; `cqrt_alloc_handle` 6; `qram is v2` ZERO), BUILD_FAIL 3 (`bd w9i`, upstream's uncommitted `cqrt_ry_<W>_controlled` — the same three as Step 28).** Residue over the 71 that complete: 13 fixtures, every one pre-existing (the loop-break family and its siblings), 936 qubits UNPROVEN over 63 unproven frees, **0 convicted, 0 dirty**. The six qram fixtures rotate their indices with `cqrt_ry` before every store and load (read out of their goldens), so on each the tape slot was released CLEAN by the CERTIFICATE and not by the shadow — the corpus witness of D24 (c). The L6 ctest entry stays red on this box for w9i alone. | 9, 15 D24 |
| 28 | **v1.1 — TAPE (`bd 1za`, PRD §15 D23, 2026-09-02).** The 11 `cqrt_tape_*` symbols move from `cq_runtime_v2.c`'s aborts into `shim/cq_runtime_tape.c`; M07 gains `CQ_SLOT_TOKEN` (§0.5); `cq_shim_record.c` gains two rows with `src` a READ; `cqrt_tape_alloc` joins the D21 alloc exemption. **Red** — `tests/test_runtime_tape.c` + `tests/test_runtime_tape_sweep.inc`, split on the seam recorded here and taken on the FIRST measurement, the `.c` having landed at 367 against the 300 wall: **`the SWEEP ↔ the ROWS`** — the width-dispatched ABI helpers, the seeded draw and the two sweep cases in the `.inc`; the token, the L5 rows, the promotion tuple, the recorder and the corpus shape in the `.c` (L1: the kept rail's value is `src`'s over random masks at `cq_kd_samples()`; L2: `live = src ∪ out ∪ ctrl` as a SET after the write AND after `cqrt_free(src)`, which must free CLEAN because the write only READS it; L5: classical `src` is 0 gates 0 qubits, also under ZERO and ONE controls, Q0 leaves `out` at 0 and Q1 at `src`, all-quantum promotion `(0, W, 0) → (0, 0, W)`; the kept rail is live at the end; the recorder models the write) + `tests/test_runtime_tape_death.c` (a rail as tape, a token to `cqrt_x`/`cqrt_free`/`cqrt_measure`, a token as `src`, a token as the flag) + a `#REGISTER` case in `test_shim_trace.c`. **Gate** — both configurations green, `make lint`, a mutation battery per the recorded harness rules (the un-copy promoted inside the region, a record row deleted, token/rail confusion), and L6 re-run: the `tape is v2` bucket is **0** and the six fixtures name where they moved. **QRAM (27) reuses the token** — same state, a shim-side payload. **LANDED 2026-09-02.** `shim/cq_runtime_tape.c` **89** of 120; `reg.c` **261** (+23; the 260 trigger fired by one line, split declined on `bd 2z1`'s precedent), `reg_check.c` 80, `cq_shim_record.c` 195; `cq_runtime_v2.c` down to 98 bodies. **RAN: Release 325/325, Debug 326/326 (L6 separate), `make lint` green. BATTERY: 25 mutants + 2 sanity (must-die, must-crash), all 27 LANDED, 0 NOOP, 0 NOCOMPILE; 23 KILLED IN BOTH CONFIGURATIONS after one pin was added** — R5, `self_adjoint = 1` on the tape row, is INERT (every write mints a fresh `out`, so no two records share handles) and is now pinned as the row's declared intent; **T11** (the mint inside the bracket) is a MEASURED EQUIVALENT recorded at the site, with its paired mutation T10 (birth ONE) killed; **M6** (the I2 sweep not skipping a token) is Debug-only code — killed in Debug, absent in Release. The un-copy mutant the Red column names does not exist for tape: there is no copy to undo, the kept rail is the point; its analogue T1 (the controlled body run OUTSIDE the region) is killed. **L6: `tape is v2` is 0 and all six run to completion with zero residue** (see Step 24 and §7 for the upstream widening that reddens the gate on the same run) | 1, 15 D23 |
...
v1 ships when NORTH_STAR's five conditions hold, each traced to a step. **ALL FIVE ARE MET AS
OF 2026-09-02.** Each row carries the date it was first measured and what this pass re-ran —
Rule 17, what was RUN and not "tests pass". Three of the five rest on opt-in trees: condition 1
and condition 4's CQ_lang arm on `-DCQOPS_CQLANG_DIR=`, condition 4's T-count and condition 5 on
`-DCQOPS_QEC_DIR=`, and this repo pins NEITHER dependency, so a MET here is a measurement on a
named box against a named revision and is to be **re-measured rather than quoted**. This pass
ran both configurations with both flags set — Release **316/316**, Debug **317/317** (ASan +
UBSan + LeakSanitizer), the L6 corpus against CQ_lang `893b769` (2026-08-30, dirty working
tree) in each — and `make lint`.

**RE-MEASURED 2026-09-02 (later the same day) AFTER STEP 28 — TAPE, PRD §15 D23.** Release
**325/325**, Debug **326/326** (L6 excluded from those counts and run separately in both
trees), `make lint` green (180 files, largest 297). **The L6 run is RED on this box and the
cause is upstream, not ours**: CQ_lang's working tree at `893b769` gained three UNTRACKED
fixtures that afternoon (`slice_control_inplace_ry{,_negated,_derived_base}`, mtime 12:46 and
14:51) whose lowered code references `cqrt_ry_i32_controlled` — a symbol the frozen ABI at
`shim/cq_runtime_abi.h` does NOT declare (`cq_runtime.h`'s own note: *there is no plain
`cqrt_ry_<W>_controlled` at any width*) and that upstream's uncommitted `runtime/cq_runtime.h`
now adds at `:229-232`. Those three are `BUILD_FAIL` at the LINK stage in both trees; every
other fixture LINKED and RAN. This repo pins the ABI by re-copying at a named revision, never
by chasing a dirty tree, so the row below is left MET for what it measured and the widening is
filed as its own bead rather than absorbed here.

**RE-MEASURED 2026-09-02 (later still) AFTER STEP 27 — QRAM, PRD §15 D24.** Release **354/354**, Debug **355/355** (ASan + UBSan + LeakSanitizer; L6 excluded from those counts and run separately in both trees), `make lint` green (193 files, largest 297). The L6 run keeps Step 28's shape exactly: the same three `BUILD_FAIL` (upstream's uncommitted `cqrt_ry_<W>_controlled`, `bd w9i`), and every other fixture LINKED and RAN — **71 OK against CQ_lang `893b769` (dirty), up from 65 by exactly the six integer qram fixtures, and the `qram is v2` bucket is ZERO**. Row 1 stays MET with the w9i caveat unchanged. Row 3 gains its third intended-safe-leak population and a corpus witness for the certificate: an array's cells are held for good (the ABI has no qram free), and the six qram fixtures' tape slots — pushed at birth `|0⟩`, popped by `store_unc` — were released CLEAN by D15's certificate on indices the goldens rotate with `cqrt_ry` before every store and load, where the shadow can prove nothing; each of the six reported zero residue in every grain, and the unit witness is `a_poisoned_index_store_unc_releases_the_slot_clean_by_the_certificate`. The residue over the 71 is unchanged in kind: 936 qubits UNPROVEN, 0 convicted.

**RE-VENDORED 2026-09-10 — THE ABI WIDENED AND THE L6 GATE IS NO LONGER RED FOR A REASON WE
DO NOT OWN (`bd w9i`, PRD §15 D16).** The two paragraphs above left the widening filed rather
than absorbed, on the ground that *"this repo pins the ABI by re-copying at a named revision,
never by chasing a dirty tree"*. **CQ_lang committed it**: `f92d95e` (2026-09-02) declares
`cqrt_ry_<W>_controlled` at all nine widths, and at `170ede1` (2026-09-10) its working tree is
CLEAN — zero porcelain lines — so the `(DIRTY WORKING TREE)` tag the L6/L7 runners print is
gone rather than merely tolerable. `shim/cq_runtime_abi.h` is re-vendored at that revision.

**THE DELTA IS PURELY ADDITIVE AND THAT WAS MEASURED, NOT ASSUMED.** 173 → 182 declarations by
comment-stripped identifier extraction on both headers: **nine pure insertions, one family, no
existing signature moved**, verified by compiling the vendored copy TOGETHER WITH upstream's
header (zero `conflicting types`, with a one-parameter-widened negative control producing
exactly one). `third_party/cq_lang/opcode_table.yaml` is **byte-identical** at this revision
(sha256 `6245117d…a3e82426`), so the 2,479 `cq_template_*` grid did not move and **nothing
under `third_party/` was touched** — the re-vendor is confined to `cq_runtime.h`, which that
directory does not carry. **`bd w9i`'s step 1 says to re-pin `third_party/cq_lang/COMMIT` as
well and that would have been wrong**: the artefact it pins did not change, so updating it
would claim a re-vendor that did not happen.

**THE POPULATION PINS WENT RED ON PURPOSE IN FIVE PLACES, AND THE BEAD'S RECIPE NAMES FOUR.**
`tests/test_runtime_abi.inc` (173 → 182, gate 30 → 35, fp 34 → 38, the census subset 139 →
144), `tests/test_runtime_v2.c` (35 → 39 deferred, fp 34 → 38), `cmake/CqopsSymbolSets.cmake`
(171 → 180 defined), `docs/cqrt_census.txt` (PART D's total and grid, D2, D4's partition and
its verbatim list) — **and `cmake/CqopsLinkWitness.cmake`'s generated
`_Static_assert(CQ_LINK_C_ROWS == 171)`**, which is the fifth and is different in kind: it is
GENERATED, so it fails at BUILD time rather than at test time, and its message names neither a
test nor the count's owner.

**WHAT WAS STALE PROSE RATHER THAN A PIN, sorted the way `bd cxp` established.** Six sites
carried the LIVE claim *"there is no plain `cqrt_ry_<W>_controlled` at any width"*, every one a
faithful reading of the ABI as vendored and every one now false — corrected in place, each
saying what it used to say and why: `shim/cq_runtime_gate.c`, `shim/cq_runtime_v2.c`,
`tests/test_runtime_gate_rotate.inc`, `PRD §2.1`, `PRD §15 D16`, `docs/cqrt_census.txt` D2.
The DATED MEASUREMENTS around them (the 173-era counts, CQ_lang's own
`core_abi_link_check.py:22` figure) are kept and marked as dated.

**DISPOSITION — §15 D16's capability rule, and the fp half is the part with judgement in it.**
Five integer widths IMPLEMENTED in `shim/cq_runtime_gate.c` (the `_controlled_inv` body without
its `-angle`); four fp widths as loud aborts in `shim/cq_runtime_v2.c`. **"Implement all nine
uniformly" was weighed and NOT taken**, and the reason is unreachability rather than taste:
`cqrt_alloc_f<W>` is itself an abort, so no fp rail handle can exist in v1, and an implemented
body would take M07's generic handle error instead of `cq_shim_unsupported`'s named one — worse
diagnostics on a path nothing can reach, a forward implemented where its own `_inv` is not, and
the only implemented fp rotation of any kind. The abort's reason string is `"fp is v2"`, which
is the claim, and is what those four bodies become when fp lands.

**NEW COVERAGE, because the forward twin is a comparison the tree could not make before.**
`tests/test_runtime_gate_rotate.inc` gains `ry_controlled_inv_is_ry_controlled_at_a_negated_angle`
(the promoted four-gate general row, all-quantum, bitwise ordered stream comparison — a
transposed `R(±θ/2)` or a forgotten negation moves this and moves no count) and
`a_controlled_ry_taints_its_target_and_a_controlled_rz_does_not`, which is a claim about the
RECORD layer: `shim/cq_shim_record.c` marks `first_rot` for the non-diagonal Ry rows **by
naming them**, so a row added to the effect table but not to that list would leave a controlled
forward Ry looking diagonal to D15's certificate and a rail it rotated could be released CLEAN.
`CQ_ROP_RY_CTRL`'s effect row is byte-identical to `CQ_ROP_RY_CTRL_INV`'s, which is the
relationship `CQ_ROP_RZ_CTRL` and `CQ_ROP_RZ_CTRL_INV` already have — the forward/inverse
distinction lives in the caller passing `-angle`, never in the table.
...
| 2 | **MET (Steps 10–17; re-run 2026-09-02, both configurations)** — every integer opcode differential-tested against C semantics at every width on its shipped ladder, over a small constant number of seeded random `(bit-kind mask, value)` samples per width, with the all-classical and all-quantum masks and the value corners forced into every draw. **What carries it:** the eleven kernel suites `test_kernel_{bitwise, shift, shift_var, cast, add, addacc, cmp, mux, mul, divrem, sdivrem}`, each enumerating its OWN ladder rather than a shared one — bitwise to 64, `icmp` to **80** (the one family in scope there), add / mux / divrem to **128**, casts over the width PAIRS the yaml ships — at `cq_kd_samples()` = **32** per `(kernel, width)` with the four anchors inside the budget (`kernelsweep.c`; `CQOPS_L1_SAMPLES` overrides, and the suite was also green at 256 on 2026-08-21). Widths are ENUMERATED and never sampled. The reference is `tests/support/refmodel.[ch]` + `refmodel_w.c` — plain C, never derived from a kernel, two-word above 64 so i80 and i128 are CHECKED rather than skipped, and a wrong request (a width off the ladder, a negative shift, a cast in the wrong direction) is a hard error rather than a silent truncation | 10–17 (L1) |
| 3 | **MET (Steps 10–17 and 21; re-run 2026-09-02, both configurations) — AND THE WORDING IS CORRECTED HERE, as `bd a0g` records for this row's four copies.** ~~after every template call the pool holds exactly the result rail's qubits; after `_unc`, nothing~~ — that sentence is true only under the all-classical mask: an operand with any `CQ_BIT_Q` bit owns live qubits that are nobody's leak. What is ASSERTED is the **union form — no index is live that no named register owns, and every owned index is live** — as a SET and never a count (`cq_pc_live_is_exactly`), at THREE points on every L1 case of every kernel under each of §9's four regions: after the forward, after `_unc`, after the free. Three points because the discriminating fault nets to zero — one ancilla acquired, one SOURCE qubit released — and is invisible to any count and to any single snapshot. After `_unc` the VALUE is zero and the qubits are still held, since `_unc` reclaims nothing and `cqrt_free` is the sole deallocator (PRD §10); after the FREE, `live` is restored and every index `dst` held is back on the free list BY INDEX (L3, `cq_pc_indices_are_free`) — never by comparing `minted`, `peak` or the free-list length, all of which are monotone. At the handle boundary `test_template.c`, `test_runtime_rail.c` and `test_runtime_gate.c` make the same set assertion through the frozen `cqrt_*` ABI. **NORTH_STAR's two intended-safe-leak populations are MEASURED, not assumed**: a rail CQ_lang never frees stays held for good, and a freed rail the certificate cannot discharge is STRANDED — readable from outside the archive since `bd c55`, and on this pass's L6 run every stranded qubit was merely UNPROVEN and not one was CONVICTED, so `ckd.18`'s provably-dirty row is reached by the unit suite and by nothing in the e2e corpus (`bd memories l6-corpus-residue`; a ratio and a shape, re-measure rather than quote). NORTH_STAR's own copy of the old sentence is still `bd a0g`'s to correct | 10–17 (L2), 21 (L3) |
| 4 | **MET 2026-08-28 (Step 25; re-run 2026-09-02, both configurations)** — ordinary C compiles through `cqc`, links, emits a gate stream whose oracle arithmetic is verified exactly in classical mode. **AMENDED BY PRD §15 D22 (2026-08-28), and the amendment is that the two halves of this sentence RUN IN DIFFERENT MODES.** *Compiles / links / emits a gate stream* is the QUANTUM arm, through CQ_lang, opt-in behind `-DCQOPS_CQLANG_DIR=`, and it needs the **non-empty** clause spelled out — a program that folded away also links, runs and does not abort, i.e. also passes L6's D18 gate. *Verified exactly in classical mode* is the CLASSICAL arm, and what it verifies is the **value** plus **L5 at program scale** (zero gates, zero qubits) — **not** "the oracle's arithmetic circuits", which are never BUILT in that mode and are carried by condition 2. **Where the three claims are measured, in three places because they run in three modes:** `l7_grover_cqlang` (the CQ_lang arm — link, run, do not abort, AND a non-empty gate MIX with `x`, `cx`, `ccx`, `ry`, `rz`, `mz` all present, since a `θ = π` mutant passes D18's gate and only that clause kills it; **plus, since 2026-09-10, a SIXTH clause the mix cannot supply** — every `rz` target disjoint from the `mz` targets, because the retired `cq_phi(x, θ)` spelling emitted a TENSORED `Rz` that was the identity on the marked branch and emitted MORE `rz` lines than the kickback form, so no count could see it: `bd cxp`, PRD §12's note (iii)); `test_grover` (the classical arm — value equals plain C at zero gates and zero qubits — plus the per-iteration closed forms `ry = W·(iters+1)`, `rz = iters·(W+1)`, which read no golden and are what survive `CQOPS_UPDATE_GOLDENS=1`); `test_grover_qec` (the T-count as a MEASUREMENT — `{Ry(π/2), Rz(π)}` costed T-free on `config_ccx.json`, with `Rz(0.1)` as the negative control — behind `-DCQOPS_QEC_DIR=`). **§12's listing does not lower through CQ_lang** — two upstream guards decline it, both CQ_lang doing its job — so the acceptance artefact is CQ_lang's own accepted spelling and the listing stays source intent (`bd memories l7-what-step-25-actually-witnessed`) | 25 |
| 5 | **MET 2026-08-28 (Step 26; re-run 2026-09-02, both configurations)** — one flag routes the same stream into `qec_*`, and the flag is `CQOPS_SINK=qec`. `cq_sink_qec_bind` is a no-op unless that sink is the one in force — that check IS the one flag — and the install is two calls on either side of `cq_ctx_init` because no instant satisfies both preconditions. `src/sink_qec.c` compiles on every box; WITHOUT `-DCQOPS_QEC_DIR=` its no-library arm registers nothing, so the flag takes M04's "names an unregistered sink" hard error rather than a silent printf. Measured on this pass with the library present: `test_sink_qec` + four deaths, `test_sink_qec_angle` + four deaths (M25b, the half that runs on every box), and `test_grover_qec`. The pool ceiling is `qec_n_logical` and is HARD (D20). The whole Grover program is NOT driven through the sink and that is scope, not omission — one Toffoli at `d = 3` is 562,564 physical gates and the program peaks near 100 logical qubits | 26 |

Plus the two this plan adds, **both holding on this pass**: `make lint` green (no hand-written
module over 300 lines — 177 files after `bd zmo`'s and `bd f8c`'s splits, 193 after Step 27, largest 297 — the FILE COUNT here is
box-local and settled as informational on 2026-09-11 (`bd a9e`), so it is not comparable across
checkouts and what this condition asserts is the LIMIT), and every kernel
traceable to a `docs/constructions/K*.md` spec extracted from a pinned Bennett.jl commit — all
fourteen (K13 and K14 since Step 27) name `980805de…` in their `Bennett source` header, matching the `commit:` line of
`third_party/bennett/COMMIT`, which the L4 golden loader hard-errors against on drift (`bd 7ew`).
