...
session on the dev box, against 194 tracked).
...
— and the count half is what went stale: the table is three files now, the third recording
no seam by decision (its row says why).
...
module over 300 lines — 177 files after `bd zmo`'s and `bd f8c`'s splits, 193 after Step 27, largest 297), and every kernel
