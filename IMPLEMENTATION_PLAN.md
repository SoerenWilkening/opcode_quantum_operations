...
module over 300 lines — 177 files after `bd zmo`'s and `bd f8c`'s splits, largest 297), and every kernel
traceable to a `docs/constructions/K*.md` spec extracted from a pinned Bennett.jl commit — all
twelve name `980805de…` in their `Bennett source` header, matching the `commit:` line of
