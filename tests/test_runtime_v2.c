...
/* THE FOUR BUCKET STRINGS, WRITTEN OUT BY HAND. They are the contract, so the
...
#define V2_R_QRAM "qram is v2: libcqops models no addressable quantum array " \
                  "at any width"
#define V2_R_TAPE "tape is v2: libcqops models no measurement tape at any width"
...
/* D16's population, derived from the NAME and from nothing else: qram and tape
 * are families and are deferred at EVERY width, so they are tested before the
 * fp token. That order IS the decision `bd vxk` recorded — `cqrt_qram_alloc_f32`
...
static const char *bucket_of(const char *name)
{
    ...
    if (strncmp(name, "cqrt_tape_", 10) == 0)    return V2_R_TAPE;
...
/* THE COUNT IS 109 AND ITS DECOMPOSITION IS PRD §15 D16's ARITHMETIC:
 * 173 declared = 32 rail + 30 gate + 2 cqrt_h* + 34 fp + 63 qram + 11 tape + 1
...
CQ_TEST(the_deferred_surface_is_exactly_the_headers_109_declarations)
{
    char want[128][V2_NAME_MAX];
    const uint32_t nw = header_deferred(want, 128u);

    CHECK_EQ((int)nw, 109);
    CHECK_EQ((int)CQ_V2_N_THUNKS, 109);
    ...
    uint32_t fp = 0u, qram = 0u, tape = 0u, hand = 0u;
    for (uint32_t i = 0; i < nw; i++) {
        ...
        else if (strcmp(b, V2_R_TAPE) == 0)  tape++;
        else if (strcmp(b, V2_R_HAND) == 0)  hand++;
    }
    CHECK_EQ((int)fp, 34);
    CHECK_EQ((int)qram, 63);
    CHECK_EQ((int)tape, 11);
    CHECK_EQ((int)hand, 1);
}
...
CQ_TEST(cqrt_h_and_its_controlled_twin_are_defined_nowhere_in_the_shim)
{
    static const char *const files[] = {
        ...
        "cq_runtime_gate.c", "cq_runtime_v2.c"
    };
...
CQ_TEST_MAIN(
    CQ_CASE(the_deferred_surface_is_exactly_the_headers_109_declarations),
