 ...
 * copies of its stub set). It is installed BEFORE the first cq_shim_ctx() so
 * the shim's built-in printf sink does not win: these cases build real rails
 * on their way to the assertion, and a death binary's stdout is not a trace.
 * The storage stays file-static here because cqops_set_sink BORROWS it. */
