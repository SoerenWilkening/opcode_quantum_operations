...
> **AMENDED AGAIN 2026-09-17 (`bd 30k`) — THE SAME DOOR, ONE CALL OVER, AND ONE OF THE
> AMENDMENT ABOVE'S OWN FIGURES HAS NOW FLIPPED TOO.** Both paragraphs are kept verbatim as
> what was measured on their dates; this is the third measurement, not an overwrite. Against
> CQ_lang `b1b1dc02` (2026-09-17, **DIRTY working tree** — modifications confined to `docs/`
> and `.gitignore`, `tests/e2e` tracked-clean) and its **392** goldens: **548**
> `cqrt_measure_*` calls, **0** later freed, and **zero** later REFERENCED. `bd tgx`'s single
> witness, `slice_control_select_bool_round2_window.expected.log`, was **DELETED upstream at
> `66428ddc`** — quarantined, not superseded: the program now *declines* under CQ_lang's own
> 2026-09-14 stale-reader ruling, and its header says no golden could be regenerated because
> a decline has no trace. **The READ relaxation stands anyway**, because it was argued from
> the contract — a read is not a write — and not from that fixture; but it is now supported
> by **no golden**, which is worth knowing before anyone cites one.
>
> **WHAT THE SAME SCAN FOUND INSTEAD IS A REPEAT MEASURE, AND IT IS WHY `cq_reg_mark_measured`
> IS NOW IDEMPOTENT.** Exactly one golden measures one handle twice —
> `slice_loop_pairing_measure_owned.expected.log`, `cqrt_measure_i32(h1)` at **line 11 and
> line 19** — and until this amendment it was the one L6 fixture that ABORTED, on
> `reg: measure of a rail that is not live (1, 3)`. **IT IS DELIBERATE UPSTREAM RATHER THAN
> TOLERATED, and that is the discriminator rather than the golden's mere existence.** The
> fixture's own header admits the rail under `test_C_libtooling-tu9q` clause (ii-c) *because*
> every user of it is a `cq_measure`; upstream's `measureOwnedInvariantRail`
> (`ir-pass/src/LoopLowering.cpp:3357`) is literally "true iff every user is a measure", and
> `FreePoint::computeFreePoint` (`ir-pass/src/FreePoint.cpp:191,204-207`) declares
> `CallInst *Measure = nullptr; // the (first) measurement sink` and `continue`s past the
> rest — code that expects plural measure users. `-O1` LICM is what produces them, hoisting
> the loop-invariant `y & 1` into the preheader while the measure stays in the two-trip body.
> There is **no uniqueness guard anywhere in the pass**: emission is one call per
> `cq_measure` site (`ir-pass/src/MeasureLowering.cpp:230-239`, `:603`).
>
> **SO THE PREDICATE SPLITS ONE MORE TIME, AND TERMINALITY LOSES NOTHING IT EVER HAD.**
> `cq_reg_mark_measured` returns unchanged on a rail that is already MEASURED; **DEAD still
> aborts there**, TOKEN and a poisoned slot one layer up in `cq_reg_slot_mut` / `cq_reg_slot`,
> the WRITE door (`cq_reg_bits`, `rail_w`, `out` in `cq_reg_check_operands`) is untouched, and
> **`cq_reg_free` of a MEASURED rail is still a hard error**. A repeat measure writes no bit,
> no pool entry and no shadow entry; its one state transition is already at its fixed point.
> **No qubit reaches the free list by any path this opens, so Rule 6 is arithmetically
> unaffected.** Upstream's own `docs/backend.md` §11.2 forbids exactly what we still forbid —
> *"a gate applied to a handle **after** its measurement is a **loud reject**"* — and is
> silent on a second measure; `docs/frontend.md` §3.4 goes further the other way (*"The handle
> is not consumed … the register is reused, not freed"*). **Those two upstream documents
> contradict each other, and CQ_lang knows**: `bd yiid` is an OPEN P1 there on post-measure
> semantics, and a ruling taken but **not implemented** (E3 "(b) BROAD") would lower a
> dominated second measure to the *classical* measure, which emits nothing. **That does not
> make today's emission illegal and it does not threaten this change**: if upstream stops
> emitting the repeat, an idempotent guard is simply never exercised. **The repeat measure
> DOES re-emit its `mz` per qubit** — suppressing it would make the gate stream depend on
> call history, which is an optimisation v1 does not do. The claim is pinned POSITIVELY at
> both layers (`tests/test_rotate_measure.inc`, `tests/test_runtime_rail.c`), because the two
> death cases it replaces would have been satisfied by merely not aborting.
...
| **D26** | **What v2 IS — the floating-point surface: which widths, which constructions, and what the finish line becomes** (filed 2026-09-14; the v2 scoping session) | **SCOPED 2026-09-14, NOTHING IMPLEMENTED. The long form is [`PRD-v2.md`](PRD-v2.md) and this row is a POINTER, not a summary — read it there.** Four things this row exists to stop being re-derived. **(1) v2 IS `f64` ONLY, and the gap is WIDTH rather than CONSTRUCTION.** Measured over the 367 goldens at CQ_lang `607b6fe4`: **zero** fp opcodes the corpus calls are absent from the pinned Bennett snapshot, so Rule 1 is fully satisfiable at the opcode level — but `softfloat/` is **binary64**, `f16` and `f80` have **zero files**, and `f32` is an explicitly double-rounding `fpext→f64→fptrunc` round trip whose own source says the bit-exactness contract *"does NOT extend to f32 arithmetic"* and that `reversible_compile(f, Float32)` **is rejected**. Taking `f32` would put a construction upstream declines to compile inside condition 2's bit-exact differential test. `f80` is a **dependency on upstream**, not a gap to close here. **(2) THE 884 IS THE WRONG UNIT, IN BOTH DIRECTIONS** — only **196** of the 884 are called at all (688 have no caller), while **75** fp symbols the corpus calls **4,913** times are **not in the grid**, dominated by **`fma` at 4,295** — the single most-called fp symbol in the corpus, from CQ_lang's two **non-vendored** tables (`intrinsic_table.yaml`, `libm_table.yaml`), which `third_party/cq_lang/COMMIT` has flagged as an open scope question since 2026-08-14 and which **PRD-v2 §6.1 RESOLVES on 2026-09-17: v2 VENDORS both tables, the complete `cq_template_*` link surface is 2,880 symbols, and the vendoring is scheduled WITH the port of the families it exposes and never ahead of it.** **Size v2 in FIXTURES**: 191 goldens touch fp and an `f64`-only scope unblocks **100** — **conditional on that vendoring**, because **76 of the 100 also call a symbol the grid does not contain**, and without it the figure is **23**. Measured the same day: **6 fixtures with NO floating point anywhere** are already DEFERRED at L6 on `cqrt_alloc_handle` for the same reason, so this was never only an fp question. **(3) THE SHADOW SURVIVES, AND IT IS PROVEN RATHER THAN ARGUED.** `third_party/bennett/BENCHMARKS.md`'s `Total` equals `NOT+CNOT+Toffoli` **exactly** on all three softfloat rows (63,058 / 149,456 / 247,398), so the construction emits `{X, CX, CCX}` and nothing else: **Rule 4 intact, D12's rotation-free surface extends over the whole fp kernel surface, the shadow stays EXACT.** That conclusion is fold-invariant; the MAGNITUDES in the same table are not, and this repo already has a recorded instance of reusing one (`x+1 i8` at `100/4/68/28`) — re-measure before pinning any golden. **(4) AN fp RAIL IS v1's REGISTER AT AN fp WIDTH — NO REPRESENTATION CHANGE, and the seam was recorded before v1 shipped**: plan §3's Layer-5 table already says the 34 deferred fp-width core symbols *"become real code by WIDENING what already exists"*. Bennett's softfloat never sees a float (`soft_fadd(a::UInt64, b::UInt64)`), **I5 and I4 are untouched so L5 survives**, and sign/exponent/mantissa are contiguous spans — K12's `r_in[t]` shape, where **guards compare ranges and not base pointers**. **NOT CHANGED BY v2: D14.** Its 603 integer `_inv` aborts stand on **specifiability**, not liveness; D14's five NAMED opcodes are **232** symbols exactly (re-measured 2026-09-14), a further **228** are non-injective by the same argument but unnamed, and the **143** that do have an inverse are called **zero** times. **AND CONDITION 5 IS UNREACHABLE FOR fp BY ARITHMETIC, WHICH IS A SCOPE STATEMENT AND NOT A FAILURE**: one `f64` `fadd` needs **29,949** ancillae against D20's measured `n_logical ∈ {1,3,4,6,7}`, and its 12,488 Toffolis are ~7.0e9 physical gates at `d = 3`. Condition 5 was met at Step 26 and is a claim about the ROUTE; what is bounded is which programs are inspectable through it **RE-MEASURED 2026-09-17 at CQ_lang `b1b1dc02` (dirty tree, 392 goldens): every fp figure in this row is unchanged** — none of the 27 added or 2 removed fixtures contains an fp `cq_template_*` call, and the `607b6fe4` totals were recomputed from `git show` to confirm it rather than inferred from the delta. What moved is three figures in PRD-v2 itself, corrected there with both values kept. **AMENDED 2026-09-18 — THE PORT'S MECHANICS ARE DECIDED, in PRD-v2 §7 (fifteen items), still with nothing implemented.** The three that a reader must not re-derive: **(a)** there is NO gate stream to copy — the snapshot ships none — so "port `soft_fadd`" means transcribe the Julia SOURCE line by line as step blocks over the K-catalogue and hand `cq_sandwich` a dispatching step function (K12's shape), LITERALLY — one block per operator occurrence, 64-bit spans, no CSE, no narrowing (§7.2), constants as SOURCES rather than scratch (§7.3); **(b)** the classical short-circuit is a C transcription of the same body over `uint64_t`, NOT the host `double` operator, because the IEEE-unspecified cells were MEASURED to be x86's choice on this box (`Inf−Inf = fff8…`, first-operand NaN, no signalling priority — Bennett's `INDEF`) and ARM's differ, so the operator would make classical and quantum mode disagree on some hosts (§7.4); L1's oracle stays the host operator with those cells pinned by table; **(c)** the transcendentals are out on ABI grounds and arrive anyway — CQ_lang's Rule 18 SOURCE-INJECTS `math.h` bodies, measured as 12 `fma` + 4 `fmul` + … for `exp2` — and the LLVM-name → `soft_*` mapping is not identity (`rint` = `soft_round`, `round` = `soft_round_away`, `fmin` = `soft_fmin` not `soft_fminimum`) (§7.7–7.8). Also there: `fptosi`/`fptoui` pin upstream's x86 saturation under D3's posture (§7.5), a per-file D8 audit of variable shifts (§7.6), three step exports v1 owes first (§7.10), the anchors §6.5 asked for (§7.12), M40 `fsqrt` added to §5, and the integer intrinsics as a v1-shaped port (§7.14). |
| **D27** | **Bit-level lowering — "thin backend, fat compiler": CQ_lang lowers arithmetic to i1 gates itself (or an expand-then-reoptimise pipeline does) and libcqops shrinks to Layers 0–1 + 4** (filed 2026-09-18; proposed, prototyped and measured in one session) | **NOT ADOPTED, 2026-09-18, ON MEASUREMENT, and recorded so the analysis is paid for once.** The harness is `tools/bitlevel/` — `gen_fixture.py` emits ONE program in TWO spellings at width `W` (`+` on an unsigned, and K6's ripple carry `t=a^b; s=t^c; c'=(a&b)^(t&c)` written over tainted `bool`s, which is exactly what a bit-level lowering would hand the pass, so neither repo needed a change); `bitlevel.py` runs both through CQ_lang's front end and pass, links against this archive, and tabulates. The long form with every table is lab-report entry 12. Measured 2026-09-18, Release archive, CQ_lang `62b0b111`, printf sink, a four-deep add chain, `-O1` (`-O2` identical; `-O0` declines in the pass for BOTH spellings, "tainted value stored to memory"). **(1) THE T-COUNT HALVES, AND IT IS NOT A PROPERTY OF BIT GRANULARITY.** Distinct addends, W=32: opcode 1568 CX / 868 CCX / peak 352 / 21 ABI calls; bit-level 1424 CX / **488** CCX / peak **776** / **2136** calls (W=8: 392/196/88/21 vs 320/104/176/480 — the ratios are width-invariant, as they must be for a linear kernel). The saving is **global-vs-local uncompute**: a Rule-7 kernel pays Bennett-in-the-small (carries built twice per call) and the pass's `_unc` then re-runs the whole sandwich (twice more) — 8 Toffolis per lane over an intermediate's life — while a value-level lowering keeps the carries alive from forward to `_unc` and pays 4. Bennett's pebble game: **store the carry chain or recompute it; the two never cancel.** The same 2× is reachable AT THE OPCODE BOUNDARY by a kernel variant that hands its scratch back as a named handle for the pass to uncompute with the result — ≈184 CX / ≈112 CCX / peak ≈100 at W=8 by K06.md's `5W−2` formula, **DERIVED, NOT MEASURED** — which is an ABI addition on CQ_lang's side and the follow-on worth prototyping if T-count ever outranks qubits. **(2) THE 2× PEAK IS TWO FACTORS, ONE OF THEM INHERENT.** SSA gives every intermediate its own rail — 4 scratch per lane against K6's 2, because two Toffolis into one target is not a value — recoverable by three rewrites the pass lacks today (XOR in place onto a dead operand; an AND consumed only by an XOR emitted as a Toffoli into that XOR's accumulator; reordering for deadness, which is K6's own gate order), after which the value-level form BEATS the kernel's CX count, since the sum is the final value of `t`'s rail and Rule 7's `dst ^= f` copy-out (W CX + W qubits per call) has no counterpart. The other factor — carries held across the gap for every live intermediate — IS the Toffoli saving of (1) and cannot be removed without giving it back. **(3) CLANG'S ALGEBRA LANDS ON THE OPCODE SPELLING, AND ONLY THERE.** Same addend four times, W=32: opcode **280 / 120 / 190** — stage 1 (`clang -O1`, which today's pipeline already runs before the pass) rewrote `x+y+y+y+y` as `x+(y<<2)` and three adders vanished; bit-level 1390 / 484 / 669 — clang cannot see an adder in 200 `and i1` + 300 `xor i1` and emits all four. Expanding to bits BEFORE value-level clang forfeits this; expanding AFTER (the second-clang-run pipeline) keeps it and, through GVN + `x^x=0` + DCE, mechanically collapses an expanded sandwich plus an expanded `_unc` down to compute-once — sound ONLY because the pass's second run re-schedules one `_unc` per surviving value. **Clang deletes an uncompute; it never optimises one**, because "these gates cancel" is not a fact about values, and stock LLVM 19 `opt -O2` on the LOWERED IR moved none of the 21 / 480 opaque calls. **(4) WHAT DID NOT MOVE**: run time (~10 ms both), compile time (0.36 vs 0.43 s clang, 0.13 vs 0.14 s pass at W=32 — 2,245 lines of lowered IR is nothing; "the IR is the circuit" bites near 10⁶ instructions per function, the wide `udiv`/`mul` tail, not adds), correctness (every row classical-correct at θ=π with zero gates), and D15's certificate (500+ i1 frees, zero strands — a data point FOR it at scale). **(5) WHAT IS GIVEN UP** is the per-kernel boundary at which Rule 2 is checkable and L1–L3 run in seconds; under bit-level the only detector of a wrong uncompute is the certificate at each free, and the constructions move into an LLVM pass whose oracle for its own output is trace goldens against a stub (D18). **Named: this is XAG synthesis + pebbling (Soeken, Meuli et al.) with clang as the XAG optimiser — and clang is a poor one, minimising x86 cost rather than AND count.** **RE-OPEN WHEN** T-count outranks qubit count for a target, or CQ_lang's pass gains the three rewrites of (2) plus an eager uncompute schedule; in either case re-run `tools/bitlevel/bitlevel.py --widths 8,32` FIRST and argue from the new table, not from this row. |
...
> >
> > **AMENDED 2026-09-14 (`bd 16q`) — RE-MEASURED AT A SECOND REVISION, NOT RE-PINNED. AND
> > THE TEXT TEST THE PARAGRAPH ABOVE PRESCRIBES IS ALSO WRONG, IN THE OPPOSITE DIRECTION.**
> >
> > **The decision stays dated 2026-08-22 and its pin stays `397c67c`.** `bd 16q` asked
> > whether the carve-out should be re-pinned to a current revision with fresh witnesses;
> > the answer is no, for this section's own reason — **nothing in this repo pins CQ_lang**,
> > so a re-pin to a HEAD that moves within days would manufacture provenance rather than
> > supply it. What follows is a dated re-measurement pointing forward, which is what §0 is
> > for.
> >
> > **(d) BOTH OF `bd 16q`'s HEADLINE CLAIMS REPRODUCE AT A SECOND, LATER REVISION.** At
> > CQ_lang **`607b6fe4`** (2026-09-14, clean tree), `tests/e2e` holds **367** goldens and
> > **60,424** `cqrt_free` calls — **+127 goldens and +8,726 frees** against the `397c67c`
> > pin. The `ckd.18` shape counts **zero**, as it did at `170ede1`; `slice_loop_break*` is
> > **four**, as `bd 16q` reported. **So the witness did not "move out from under" the
> > carve-out between those two revisions — it was already absent at both, and the shape's
> > last non-zero reading is the pin's own.**
> >
> > **AND §4's MECHANISM REPRODUCES EXACTLY, WHICH IS THE PART THAT MATTERS.** Of the **32**
> > non-zero-born rails that are freed at all, **not one takes any rotation** — the same
> > fact, at a second revision, under a third counting method. The population is zero
> > because the rotated rails are never freed, and **not** because the corpus stopped
> > rotating: **389** non-zero-born rails still take at least one `cqrt_ry` and **22** take
> > exactly two. Only the second reading would let §4 be retired, and it is still false.
> > **D15's ARGUMENT never depended on these counts and does not now** — this is a
> > confirmation with a method correction, not a re-opening.
> >
> > **(e) THE PARSING TRAP HAS A SECOND FACE AND THE PARAGRAPH ABOVE WALKED INTO IT.**
> > *"Classify a literal by its text"* fixed awk's coercion and introduced the opposite
> > error. Measured over the same 30,704 `cqrt_alloc` lines at `607b6fe4`, three methods:
> >
> > | Method | Count | Fault |
> > |---|---:|---|
> > | literal TEXT `!= "0"` — what this section prescribes | **3,218** | **OVER-counts by 2,187.** `0x0p+0` is textually not `"0"` and is numerically **zero**; it occurs **2,186** times |
> > | `float.fromhex()` then `!= 0.0` | **1,031** | **UNDER-counts by one.** `0x8p-16448` is an `f80` subnormal that **underflows to `0.0` in a `double`** |
> > | significand-digit test (zero iff every significand digit is `0`), `inf`/`nan` handled | **1,032** | correct |
> >
> > So the `3,146` above is the text method at `170ede1` and is inflated ~3× by the same
> > `0x0p+0` population; the honest figure at `607b6fe4` is **1,032** (integer **474**, fp
> > **558**). **THE CONCLUSION THAT PARAGRAPH DREW IS UNTOUCHED** — it rests on *"of the
> > freed non-zero-born rails, not one is rotated"*, a ratio over a set the miscount does
> > not reorder — which is why this is amended in place rather than rewritten, exactly as
> > **D20**'s `7,300` was. **The lesson is the general one: an fp literal cannot be
> > classified by text OR by `double`**, because the literal space has IEEE structure that
> > both readings flatten. The corpus allocates **`inf` 102 times and `nan` 9 times**, which
> > is measured support for **PRD-v2 §3.4 / R12** — L1's fp anchors must be the IEEE special
> > classes, not `0` and `~0`. `bd remember fp-literals-cannot-be-classified-by-text-or-by-double`.
>
