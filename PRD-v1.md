...
> (`if (cond) x = cq_phi(x, M_PI);`) whose flag and work rail the region teardown strips
...
> the same day: D24 moved the 63 `qram` bodies out too**, so the file holds the 34 fp-width
> core symbols and `cqrt_alloc_handle` and nothing else. `nm` still shows
> 171 defined `cqrt_*` — they were defined before, as aborts.)*
>
